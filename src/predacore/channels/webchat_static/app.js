...
// PredaCore | Iron Man Inspired | Living Dashboard
// ═══════════════════════════════════════════════════════════════════
(function () {
    'use strict';
    ...
    function connect() {
        ...
        ws.onopen = () => {
            ...
            if (window.PREDACORE.sounds) window.PREDACORE.sounds.boot();
        ...
        switch (data.type) {
            ...
            case 'message':
                ...
                if (data.role === 'assistant') {
                    ...
                    if (window.PREDACORE.sounds) window.PREDACORE.sounds.chime();
                } else {
            ...
            case 'thinking':
                ...
                if (window.PREDACORE.sounds) window.PREDACORE.sounds.humStart();
                break;
            ...
            case 'tool_start':
                ...
                if (window.PREDACORE.sounds) window.PREDACORE.sounds.whoosh();
                break;

            case 'tool_end':
                logActivity('tool-end', `${data.name} \u2014 ${data.duration_ms}ms`);
                updateToolRegistry(data.name, 'completed');
                if (window.PREDACORE.sounds) window.PREDACORE.sounds.ding();
                break;
            ...
            case 'error':
                ...
                if (window.PREDACORE.sounds) window.PREDACORE.sounds.error();
                break;
        }
    }
    ...
    function renderWidgetInChat(data) {
        removeWelcome();
        if (window.PREDACORE.renderWidget) {
            const widgetEl = window.PREDACORE.renderWidget(data);
            if (widgetEl) {
    ...
    function setProcessing(active, text) {
        ...
        if (active) {
            ...
            if (window.PREDACORE.sounds) window.PREDACORE.sounds.humStop();
        }
    }
        ...
        (data.catalog || []).forEach(ch => {
            ...
            if (isActive) {
                ...
                item.innerHTML = `<span class="channel-dot dim">\u25CB</span>
                    <span class="channel-name">${escapeHtml(ch.name)}</span>
                    <span class="channel-cta" data-connect="${escapeHtml(ch.name)}">ASK PREDACORE</span>`;
            }
    ...
    function sendMessage() {
        ...
        if (window.PREDACORE.sounds) window.PREDACORE.sounds.click();
    }
    ...
    if (soundToggle && window.PREDACORE.sounds) {
        updateSoundIcon();
        soundToggle.addEventListener('click', () => {
            window.PREDACORE.sounds.toggle();
            updateSoundIcon();
        });
    }
    function updateSoundIcon() {
        if (soundIcon) {
            soundIcon.textContent = window.PREDACORE.sounds.enabled ? '\uD83D\uDD0A' : '\uD83D\uDD07';
        }
    }
    ...
    // ═══ EXPOSE ON NAMESPACE ═══
    window.PREDACORE.sendCommand = sendCommand;
    window.PREDACORE.addAssistantMessage = addAssistantMessage;
    window.PREDACORE.logActivity = logActivity;
    window.PREDACORE.scrollToBottom = scrollToBottom;
    window.PREDACORE.showToast = showToast;
    window.PREDACORE.setProcessing = setProcessing;
    window.PREDACORE.isProcessing = () => isProcessing;
