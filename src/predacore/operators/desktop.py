"""
High-power macOS desktop operator for PredaCore.
...
except ImportError:  # pragma: no cover - fallback when imported as src.predacore.*
    from src.predacore.operators.native_service import (  # type: ignore
        DesktopNativeError,
        MacDesktopNativeService,
    )

try:
    from predacore.operators.smart_input import SmartInputEngine
except ImportError:  # pragma: no cover
    try:
        from src.predacore.operators.smart_input import SmartInputEngine  # type: ignore
    except ImportError:
...
class AsyncBridge:
    ...
    def __init__(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run_loop, daemon=True, name="predacore-async-bridge"
        )
        self._thread.start()
...
class MacDesktopOperator(BaseOperator):
    ...
    def __init__(self, log: logging.Logger | None = None, operators_config: Any = None) -> None:
        ...
        # osascript per call). Set PREDACORE_DESKTOP_BACKEND=native,legacy to
        # re-enable the slow fallback if native PyObjC is unavailable.
        backend_pref = str(os.getenv("PREDACORE_DESKTOP_BACKEND", "native")).strip()
    ...
    def _dispatch_action(self, action: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if action == DesktopAction.MOUSE_CLICK:
            ...
            raise DesktopControlError(
                ...
                "Set PREDACORE_DESKTOP_BACKEND=native,legacy and grant Accessibility access."
            )
        else:
    ...
    def _screenshot(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if target:
            ...
            fd, temp_path = tempfile.mkstemp(prefix="predacore_screen_", suffix=suffix)
            os.close(fd)
    ...
    def health_check(self, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if not osascript_ok:
            hints.append(
                "Grant Terminal/PredaCore Automation + Accessibility permissions in "
        ...
        if self._native_service is not None:
            try:
                ...
                if not bool(native_check.get("accessibility_trusted", False)):
                    hints.append(
                        "For AX actions, enable Accessibility for the Python/PredaCore process "
    ...
    def _check_allowed_app(self, app_name: str) -> None:
        ...
        if app_name.strip().lower() not in self._allowed_apps:
            raise DesktopControlError(
                f"app '{app_name}' blocked by PREDACORE_DESKTOP_ALLOWED_APPS policy"
            )
