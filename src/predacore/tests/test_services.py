"""
Comprehensive tests for predacore.services — Services Layer (Phase 7).
...
# PIDManager
# ---------------------------------------------------------------------------
from predacore.services.daemon import PIDManager
...
class TestPIDManager:
    ...
    def test_write_creates_pid_file(self, tmp_path):
        pid_path = tmp_path / "predacore.pid"
    ...
    def test_write_creates_parent_dir(self, tmp_path):
        pid_path = tmp_path / "sub" / "dir" / "predacore.pid"
        mgr = PIDManager(str(pid_path))
        mgr.write()
        assert pid_path.exists()
...
# CronEngine
# ---------------------------------------------------------------------------
from predacore.services.cron import CronEngine, CronExpression, CronJob
...
# LaneQueue
# ---------------------------------------------------------------------------
from predacore.services.lane_queue import LaneQueue, LaneTask, TaskStatus
...
# OutcomeStore
# ---------------------------------------------------------------------------
from predacore.services.outcome_store import OutcomeStore, TaskOutcome, detect_feedback
...
# TranscriptWriter
# ---------------------------------------------------------------------------
from predacore.services.transcripts import TranscriptEntry, TranscriptWriter
...
# IdentityService
# ---------------------------------------------------------------------------
from predacore.services.identity_service import IdentityService
...
# Plugins
# ---------------------------------------------------------------------------
from predacore.services.plugins import (
    Plugin,
    PluginRegistry,
    hook,
    tool,
