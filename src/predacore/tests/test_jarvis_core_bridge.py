"""
Bridge tests for predacore_core (Rust extension via pyo3).
...
class TestVectorOps:
    ...
    def test_cosine_similarity_identical_vectors(self):
        v = [1.0, 0.0, 0.0]
        assert math.isclose(predacore_core.cosine_similarity(v, v), 1.0, abs_tol=1e-6)

    def test_cosine_similarity_orthogonal(self):
        a = [1.0, 0.0, 0.0]
        b = [0.0, 1.0, 0.0]
        assert math.isclose(predacore_core.cosine_similarity(a, b), 0.0, abs_tol=1e-6)

    def test_cosine_similarity_opposite(self):
        a = [1.0, 0.0, 0.0]
        b = [-1.0, 0.0, 0.0]
        assert math.isclose(predacore_core.cosine_similarity(a, b), -1.0, abs_tol=1e-6)
    ...
    def test_l2_normalize_unit_length(self):
        ...
        normalized = predacore_core.l2_normalize(v)
    ...
    def test_l2_normalize_zero_vector_safe(self):
        ...
        result = predacore_core.l2_normalize([0.0, 0.0, 0.0])
        assert len(result) == 3
    ...
    def test_vector_search_finds_nearest(self):
        ...
        results = predacore_core.vector_search(query, vectors, 2)
    ...
    def test_vector_search_top_k_clamped_to_corpus_size(self):
        ...
        results = predacore_core.vector_search(query, vectors, 10)
        assert len(results) == 2  # clamped to corpus size

    def test_vector_search_empty_corpus(self):
        results = predacore_core.vector_search([1.0, 0.0], [], 5)
        assert results == []
    ...
    def test_vector_search_parallel_threshold(self):
        ...
        results = predacore_core.vector_search(query, vectors, 5)
...
class TestBM25:
    ...
    def test_tokenize_basic(self):
        tokens = predacore_core.tokenize("Hello World Python")
        assert "hello" in tokens
        assert "world" in tokens
        assert "python" in tokens

    def test_tokenize_lowercases(self):
        tokens = predacore_core.tokenize("PredaCore IS GREAT")
        for t in tokens:
            assert t == t.lower()

    def test_tokenize_min_length(self):
        """Tokens shorter than 2 chars are dropped (per Rust side)."""
        tokens = predacore_core.tokenize("a ab abc")
        assert "a" not in tokens
        assert "ab" in tokens
        assert "abc" in tokens

    def test_tokenize_empty(self):
        assert predacore_core.tokenize("") == []
    ...
    def test_bm25_search_finds_exact_match(self):
        ...
        results = predacore_core.bm25_search("python", documents, 3)
    ...
    def test_bm25_search_returns_descending_scores(self):
        ...
        results = predacore_core.bm25_search("apple", documents, 6)
        # Scores should be monotonic non-increasing
        scores = [r[1] for r in results]
        assert scores == sorted(scores, reverse=True)

    def test_bm25_search_empty_corpus(self):
        results = predacore_core.bm25_search("python", [], 5)
        assert results == []

    def test_bm25_search_empty_query(self):
        # Empty query should not crash
        results = predacore_core.bm25_search("", ["doc 1", "doc 2"], 5)
...
class TestFuzzy:
    ...
    def test_trigram_similarity_identical(self):
        assert predacore_core.trigram_similarity("python", "python") == 1.0

    def test_trigram_similarity_different(self):
        # Completely different → low similarity
        assert predacore_core.trigram_similarity("python", "xyz123") < 0.3
    ...
    def test_trigram_similarity_typo(self):
        ...
        """
        sim = predacore_core.trigram_similarity("python", "pythan")
        assert sim > 0.3
    ...
    def test_trigram_similarity_is_symmetric(self):
        a, b = "claude", "clause"
        assert math.isclose(
            predacore_core.trigram_similarity(a, b),
            predacore_core.trigram_similarity(b, a),
            abs_tol=1e-6,
        )

    def test_fuzzy_match_finds_typo(self):
        candidates = ["python", "rust", "javascript", "golang"]
        results = predacore_core.fuzzy_match("pythan", candidates, 0.3)
    ...
    def test_fuzzy_match_respects_threshold(self):
        ...
        results = predacore_core.fuzzy_match("python", candidates, 0.99)
        assert len(results) == 1
        assert results[0][0] == 0
    ...
    def test_fuzzy_search_ranks_by_similarity(self):
        ...
        results = predacore_core.fuzzy_search("python", docs, 5, 0.2)
        assert len(results) >= 1
...
class TestSynonyms:
    ...
    def test_expand_synonyms_unknown_term_returns_itself(self):
        expanded = predacore_core.expand_synonyms(["unknownxyz123"])
        assert "unknownxyz123" in expanded

    def test_expand_synonyms_multiple_terms(self):
        expanded = predacore_core.expand_synonyms(["config", "database"])
        assert "config" in expanded
        assert "database" in expanded

    def test_get_synonyms_returns_list(self):
        result = predacore_core.get_synonyms("config")
        assert isinstance(result, list)

    def test_are_synonyms_self(self):
        # A term is its own synonym
        assert predacore_core.are_synonyms("config", "config") is True

    def test_are_synonyms_unrelated(self):
        assert predacore_core.are_synonyms("python", "kitchen") is False
...
class TestRelations:
    ...
    def test_classify_relation_returns_tuple(self):
        result = predacore_core.classify_relation(
    ...
    def test_classify_all_relations_returns_list(self):
        results = predacore_core.classify_all_relations(
...
class TestEmbedding:
    ...
    def test_embedding_dim_is_384(self):
        ...
        assert predacore_core.embedding_dim() == 384

    def test_is_model_loaded_returns_bool(self):
        assert isinstance(predacore_core.is_model_loaded(), bool)
    ...
    def test_embed_skipped_if_model_not_cached(self):
        ...
        We don't want CI runs to always pay that cost. Only run if:
          ...
          2. PREDACORE_TEST_EMBED env var is set (explicit opt-in)
        """
        import os
        if not predacore_core.is_model_loaded() and not os.getenv("PREDACORE_TEST_EMBED"):
            pytest.skip(
                "Skipping embed test — BGE-small model not loaded. "
                "Set PREDACORE_TEST_EMBED=1 to force a download + run, or run the "
                "longmemeval benchmark once first to cache the model."
            )

        vectors = predacore_core.embed(["hello world", "goodbye world"])
        ...
        sim = predacore_core.cosine_similarity(vectors[0], vectors[1])
...
class TestPyBridgeSurface:
    ...
    def test_version_attribute(self):
        assert predacore_core.__version__
    ...
    def test_all_bindings_callable(self):
        ...
        for name in expected:
            assert hasattr(predacore_core, name), f"missing pyo3 binding: {name}"
            assert callable(getattr(predacore_core, name))
