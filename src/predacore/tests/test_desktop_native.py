...
Usage:
    ...
    python -m pytest predacore/tests/test_desktop_native.py -v
"""
...
def native_service():
    ...
    from predacore.operators.native_service import MacDesktopNativeService
...
def desktop_operator():
    ...
    os.environ["PREDACORE_DESKTOP_BACKEND"] = "native"
...
class TestForceQuit:
    def test_force_quit_nonexistent(self, native_service):
        from predacore.operators.native_service import DesktopNativeError
...
class TestClipboard:
    def test_clipboard_write_and_read(self, native_service):
        test_text = f"PredaCore desktop test {time.time()}"
