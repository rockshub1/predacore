...
Fire stolen from the gods, given to all.  Every PredaCore instance that learns
...
Architecture:
  - Local pool:  ~/.predacore/flame/local/   (per-instance storage)
  - Shared pool: ~/.predacore/flame/shared/  (collective repository)
...
class Flame:
    ...
    Each PredaCore instance has a local copy of the Flame.  Skills flow:
      Local endorsed → shared pool → other instances' local pools.

    The shared pool can be backed by:
      - Filesystem (default: ~/.predacore/flame/shared/)
