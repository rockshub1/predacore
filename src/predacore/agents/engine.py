"""
PredaCore Agent Engine — Unified agent orchestration.

Merges the best of three systems:
- Typed coding-agent systems: typed agents with scoped tool access +
  specialized prompts
- DAF:          Agent registry, capability routing, task store, self-optimization
- PredaCore:       32 real tools, memory, screen vision, Android, voice
...
# ── The registry: real agent types with real PredaCore tools ────────────
...
def compile_dynamic_agent_prompt(
    ...
    *,
    ...
    Every dynamic/DAF sub-agent receives SOUL_SEED invariants as the leading
    block of its system prompt — the safety floor is not optional, even for
    specialized collaboration roles. A sub-agent must never operate without
    the same guardrails the top-level agent runs under.
    """
...
class AgentEngine:
    ...
    Combines typed agents, capability routing (DAF style),
    and PredaCore's real tool execution into one in-process system.
    """
