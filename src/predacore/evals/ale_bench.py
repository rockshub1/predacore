"""
ALE-bench: Agent-Level Evaluation harness for PredaCore.
...
class AgentEvalRunner:
    ...
    async def run_default(self) -> AgentEvalReport:
        """Run the built-in SAMPLE_SCENARIOS with a minimal stub agent.

        The stub agent invokes each scenario's ``expected_tools`` in order
        with mock payloads and returns an output that echoes the expected
        outcome. This exercises the full benchmark pipeline (scoring,
        aggregation, reporting) without requiring an LLM or paid API calls
        — a sanity check that the harness still runs after code changes.
        Real agent quality evaluation should use ``run_suite`` with a
        live agent_fn that drives the PredaCore core.
        """
        async def _stub_agent(prompt: str) -> tuple[list[AgentAction], str]:
            # Match prompt to the nearest scenario so we can echo its
            # expected tools + outcome. Fallback: no tools, empty output.
            match = next(
                (s for s in SAMPLE_SCENARIOS if s.prompt == prompt),
                None,
            )
            if match is None:
                return ([], "")
            actions = [
                AgentAction(
                    tool_name=tool,
                    input_data='{"stub": true}',
                    output_data="stub result",
                    latency_ms=5.0,
                )
                for tool in match.expected_tools[: match.max_steps]
            ]
            return actions, match.expected_outcome
        return await self.run_suite(SAMPLE_SCENARIOS, _stub_agent)
