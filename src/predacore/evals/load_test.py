"""
Load Tester — Stress-test PredaCore endpoints and measure performance.
...
logger = logging.getLogger("predacore.evals.load_test")
...
class LoadTestRunner:
    ...
    async def run_default(
        ...
        *,
        ...
        """Run a default load test with a simulated-latency request_fn.

        The stub request_fn sleeps for a jittered 20-80ms and returns a
        200 response — mimics a healthy in-process roundtrip. This
        validates the load-test plumbing itself (percentile math,
        concurrency scheduling, reporting) without needing a real
        running daemon. For real daemon load-testing, call ``run`` with
        an httpx-based request_fn pointed at the webchat endpoint.
        """
        import random as _r

        async def _stub(user_id: int) -> RequestResult:
            start = time.time()
            await asyncio.sleep(0.02 + _r.random() * 0.06)
        ...
        return await self.run(_stub, cfg)
