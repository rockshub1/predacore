"""
PredaCore Memory Consolidator — background memory maintenance.
...
predacore_core (Rust) is a HARD dependency. No Python heuristic fallbacks.
"""
...
class MemoryConsolidator:
    ...
    async def _extract_entities_from_text(self, text: str) -> list[dict[str, Any]]:
        ...
        Primary: predacore_core.extract_entities (Rust — 3-tier dict+regex+stopwords,
        ...
        raw = predacore_core.extract_entities(text)
    ...
    async def auto_link(self) -> int:
        ...
        - For each new pair, pass the sample sentence to predacore_core.classify_relation
        ...
        for (eid1, eid2), info in cooccurrences.items():
            ...
            rel_type, confidence = predacore_core.classify_relation(sample, e1["name"], e2["name"])
