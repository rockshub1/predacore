...
Complete technical reference for the 8 PredaCore core files that form the central nervous system of the agent framework. Every class, method, function, constant, and data flow is documented.
...
**Path**: `src/predacore/__init__.py`

### File Overview

Package root for the PredaCore agent. Contains only the module docstring and version constant. The docstring declares the three primary entry modes.

### Module-Level Constants

| Name | Type | Value | Description |
|------|------|-------|-------------|
| `__version__` | `str` | `"0.1.0"` | Semantic version of the PredaCore package |
...
Declares that PredaCore wraps the full stack (CSC, DAF, WIL, EGM, KN) into a single-process conversational agent.

---

## 2. `core.py`

**Path**: `src/predacore/core.py`

### File Overview

The "brain" of PredaCore. Orchestrates the full agent loop: receive user message, build prompt, call LLM, execute tools, apply persona drift guard, and return response. All subsystems are imported from their own modules; this file is the slim orchestrator.
...
**Internal**:
- `.config.PredaCoreConfig` -- master configuration
...
- All providers currently use `budget_tokens=36_000`, `history_min_tokens=6_000`
...
#### Constructor: `__init__(self, config: PredaCoreConfig)`

| Parameter | Type | Description |
|-----------|------|-------------|
| `config` | `PredaCoreConfig` | Master configuration object |

**Instance Variables**:

| Variable | Type | Description |
|----------|------|-------------|
| `config` | `PredaCoreConfig` | Stored config |
...
| `_tool_timeout` | `int` | Per-tool timeout in seconds (env: `PREDACORE_TOOL_TIMEOUT_SECONDS`, default 60) |

**Constructor Logic**:
1. Propagates OpenClaw/marketplace flags to env for subprocess handlers
2. Creates `LLMInterface` and `ToolExecutor`
3. Wires the tool dispatcher and provider router for in-process execution
...
Builds the canonical set of identity anchor terms used by drift scoring. Includes config name, "predacore", "prometheus", and "predacore". Also reads comma-separated terms from `PREDACORE_PERSONA_ANCHORS` env var.
...
Generates the system instruction injected when drift is detected. Instructs the model to regenerate with persona alignment, mentioning drift triggers and requirements (speak as PredaCore, don't deny capabilities, etc.).
...
Fetches relevant persistent memories to ground the current turn. Delegates to `MemoryRetriever.build_context()` (see `memory/retriever.py`), which builds a 5-section budgeted context block: preferences, entity context, semantic search, fuzzy matches, recent episodes. Max tokens configurable via env `PREDACORE_MEMORY_PROMPT_MAX_TOKENS` (default 3000).

The retriever uses `predacore_core` (Rust) for all hot compute — SIMD cosine vector search, BM25 keyword fallback, trigram fuzzy matching, synonym expansion. predacore_core is a hard dependency; there is no Python fallback path.
...
4. **System prompt**: Use the pre-built identity prompt (seed + event horizon + identity layers); defaults carry their own first-turn instructions so no separate bootstrap branch is needed
...
7. **Agent loop** (up to `_max_tool_iterations` iterations):
   ...
   e. **Provider tool tracking**: Capture tools executed internally by the LLM provider (e.g. server-side tool loops)
...
Returns comprehensive core status dictionary including: provider, model, trust level, tools count, memory items, max tool iterations, persona drift guard config, OpenClaw status, world model stats.

---

## 3. `gateway.py`

**Path**: `src/predacore/gateway.py`

### File Overview

The "front door" of PredaCore. All messages from all channels (CLI, Telegram, Discord, Web) flow through the Gateway. It normalizes messages, resolves sessions, submits to the lane queue for serial execution, and returns responses.
...
- `.config.PredaCoreConfig` -- configuration
...
#### Constructor: `__init__(self, config: PredaCoreConfig, process_fn: Callable)`
...
**Path**: `src/predacore/config.py`

### File Overview

Three-layer configuration system. Priority: env vars -> YAML file -> defaults. Produces a fully typed `PredaCoreConfig` dataclass.

### Module-Level Constants

| Name | Type | Value | Description |
|------|------|-------|-------------|
| `DEFAULT_HOME` | `Path` | `~/.predacore` | Default home directory |
| `DEFAULT_CONFIG_FILE` | `Path` | `~/.predacore/config.yaml` | Default config path |
| `DEFAULT_PROFILE` | `str` | `"balanced"` | Default launch profile |
| `DEFAULT_AGENT` | `str` | `"default"` | Default active agent |
...
- **`PredaCoreConfig`**: Master config with all sub-configs plus name, version, mode, agent, and path fields. Properties: `agents_dir`, `agent_dir`, `flame_dir`. `__post_init__` sets computed path defaults.
...
- `_dict_to_config(data)`: Dict to PredaCoreConfig conversion
- **`load_config(config_path, profile_override)`**: Main entry point. Layers: empty -> YAML -> env -> profile defaults -> PredaCoreConfig. Creates directories, warns on permissions, syncs env.
...
**Path**: `src/predacore/sessions.py`
...
- **`PredaCoreCompleter(Completer)`**: Context-aware completions for `/` commands and `/model` models.
...
- **`_handle_command(cmd, ...)`**: 13 slash commands: /exit, /status, /tools, /sessions, /clear, /model (interactive selector), /retry, /copy (pbcopy), /cost, /new, /compact, /history, /help. Bare `/` opens picker.
...
- `LLMError(PredaCoreError)` -- base, adds `provider`, `model`
...
- `DesktopError(PredaCoreError)` -- base
