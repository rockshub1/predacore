# PredaCore Public Release — Master Ship Plan
...
upgrade: a Rust compute kernel (`predacore_core_crate`) now owns all hot
memory operations, replacing the Python numpy/BM25/heuristic paths.

**Completed in this upgrade pass:**

- `predacore_core` Rust crate built and installed (BGE-small-en-v1.5 embedding,
  SIMD vector search, BM25, trigram fuzzy, 3-tier entity extraction,
  window-aware relation classification)
- `predacore_core` is now a **HARD dependency** — no Python fallbacks, no
...
- Dead code deleted: `mamba_state.py`, `predacore_core_crate/src/graph.rs`,
  `predacore/knowledge/`
- Duplicate Python BM25 in `store.py._recall_keyword` replaced with
  `predacore_core.bm25_search`
- Python heuristic entity extraction in `consolidator.py` replaced with
  `predacore_core.extract_entities` (+ LLM enrichment as bonus)
- `auto_link()` now uses `predacore_core.classify_relation` on the sentence
...
- `pyproject.toml` bumped to 0.1.0, `predacore_core>=0.1.0` as hard dep
...
- [ ] PyPI publish: `predacore_core` first, then `predacore`
...
- [ ] Write `tests/test_tool_handlers.py`
  ...
  - identity: read, update, journal
  - cron: schedule, list, delete
  - pipeline: execute, resume, save
  - collective_intelligence: status, sync, evolve, scan, endorse

### 4C: tools/ — Pipeline, MCP client, Marketplace
...
- [ ] Write `tests/test_tools_advanced.py`
  ...
  - MCP client: JSON-RPC stdio, consumes third-party MCP servers
...
- [ ] Write `tests/test_identity.py`
  - IdentityEngine: file management, default seeding
  - SOUL_SEED immutability
  - SOUL/IDENTITY/USER evolution
  - JOURNAL append-only
  - Prompt assembly (seed + identity + user + journal)
  - Default seeding (8 default files auto-copied on first run)
  - Flame skill network (collective_intelligence handlers)
...
- [ ] Entry points: `prometheus = "predacore.cli:main"`
- [ ] Verify `pip install -e .` works clean

### 11C: README.md
- [ ] Hero section (what is PredaCore, what can it do)
