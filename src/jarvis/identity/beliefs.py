...
class BeliefStore:
    ...
    def promote(self, belief_id: str) -> str:
        ...
        if not belief:
            raise KeyError(f"No belief with id {belief_id}")
        if belief.state == BeliefState.DEMOTED.value:
            raise ValueError("Cannot promote a demoted belief directly — re-add as observation")

        ladder = [
            BeliefState.OBSERVATION.value,
            BeliefState.WORKING_THEORY.value,
            BeliefState.TESTED.value,
            BeliefState.COMMITTED.value,
        ]
        try:
            idx = ladder.index(belief.state)
        except ValueError:
            idx = 0
        new_state = ladder[min(idx + 1, len(ladder) - 1)]
        if new_state != belief.state:
            belief.notes.append(f"{_now_iso()}: manually promoted {belief.state} → {new_state}")
            belief.state = new_state
            belief.last_updated = _now_iso()
            self._save()
        return belief.state
