...
# ── Defaults ──────────────────────────────────────────────────────────
DEFAULT_HOME = Path.home() / ".prometheus"
...
class DaemonConfig:
    ...
    db_socket_path: str = ""  # Unix socket for DB service (default: ~/.prometheus/db.sock)
...
class JARVISConfig:
    ...
    agent: str = DEFAULT_AGENT  # Active agent (folder name under ~/.prometheus/agents/)
    ...
    def agents_dir(self) -> str:
        """Path to the agents directory (~/.prometheus/agents/)."""
        return str(Path(self.home_dir) / "agents")

    @property
    def agent_dir(self) -> str:
        """Path to the active agent's directory (~/.prometheus/agents/{agent}/)."""
        return str(Path(self.home_dir) / "agents" / self.agent)

    @property
    def flame_dir(self) -> str:
        """Path to the Flame skill network directory (~/.prometheus/flame/)."""
...
def _sync_runtime_policy_env(cfg: JARVISConfig) -> None:
    """
    ...
    """
    os.environ["JARVIS_PROFILE"] = cfg.launch.profile
    ...
    os.environ["JARVIS_ENABLE_PERSONA_DRIFT_GUARD"] = (
        "1" if cfg.launch.enable_persona_drift_guard else "0"
    )
    os.environ["JARVIS_PERSONA_DRIFT_THRESHOLD"] = str(
        cfg.launch.persona_drift_threshold
    )
    os.environ["JARVIS_PERSONA_DRIFT_MAX_REGENS"] = str(
        cfg.launch.persona_drift_max_regens
    )


def _env_overrides() -> dict[str, Any]:
    """Read JARVIS_* environment variables and map to config keys."""
    overrides: dict[str, Any] = {}

    env_map = {
        "JARVIS_MODE": ("mode",),
        "JARVIS_NAME": ("name",),
        "PROMETHEUS_HOME": ("home_dir",),
        "JARVIS_HOME": ("home_dir",),  # Backward compat
        "JARVIS_AGENT": ("agent",),
        "JARVIS_CHANNELS": ("channels", "enabled"),
        "JARVIS_CHANNELS_ENABLED": ("channels", "enabled"),
        "JARVIS_PROFILE": ("launch", "profile"),
        "JARVIS_APPROVALS_REQUIRED": ("launch", "approvals_required"),
        "JARVIS_EGM_MODE": ("launch", "egm_mode"),
        "JARVIS_DEFAULT_CODE_NETWORK": ("launch", "default_code_network"),
        "JARVIS_ENABLE_OPENCLAW_BRIDGE": ("launch", "enable_openclaw_bridge"),
        "JARVIS_ENABLE_PLUGIN_MARKETPLACE": ("launch", "enable_plugin_marketplace"),
        "JARVIS_ENABLE_SELF_EVOLUTION": ("launch", "enable_self_evolution"),
        "JARVIS_MAX_SPAWN_DEPTH": ("launch", "max_spawn_depth"),
        "JARVIS_MAX_SPAWN_FANOUT": ("launch", "max_spawn_fanout"),
        "JARVIS_MAX_TOOL_ITERATIONS": ("launch", "max_tool_iterations"),
        "JARVIS_ENABLE_PERSONA_DRIFT_GUARD": (
            "launch",
            "enable_persona_drift_guard",
        ),
        "JARVIS_PERSONA_DRIFT_THRESHOLD": ("launch", "persona_drift_threshold"),
        "JARVIS_PERSONA_DRIFT_MAX_REGENS": ("launch", "persona_drift_max_regens"),
        "JARVIS_TRUST_LEVEL": ("security", "trust_level"),
        ...
        "JARVIS_AGENT_LLM_PROVIDER": ("agent_llm", "provider"),
        "JARVIS_AGENT_LLM_MODEL": ("agent_llm", "model"),
        "JARVIS_AGENT_LLM_FALLBACK_MODELS": ("agent_llm", "fallback_models"),
        "JARVIS_AGENT_LLM_FALLBACK_PROVIDERS": ("agent_llm", "fallback_providers"),
        "JARVIS_AGENT_LLM_TEMPERATURE": ("agent_llm", "temperature"),
        "JARVIS_AGENT_LLM_REASONING": ("agent_llm", "reasoning_effort"),
        ...
        "JARVIS_MACRO_MAX_STEPS": ("operators", "macro_max_steps"),
        "JARVIS_MACRO_MAX_DEPTH": ("operators", "macro_max_depth"),
        "JARVIS_AX_DEFAULT_DEPTH": ("operators", "ax_default_depth"),
        "JARVIS_AX_MAX_DEPTH": ("operators", "ax_max_depth"),
        "JARVIS_AX_MAX_CHILDREN": ("operators", "ax_max_children"),
        "JARVIS_SCREENSHOT_MAX_B64": ("operators", "screenshot_max_b64_bytes"),
        "JARVIS_SLEEP_MAX_SECONDS": ("operators", "sleep_max_seconds"),
        "JARVIS_SCROLL_MAX": ("operators", "scroll_max_amount"),
        ...
        "JARVIS_DB_SOCKET": ("daemon", "db_socket_path"),
    }
    ...
    for env_var, path in env_map.items():
        ...
        if value is not None:
            ...
            # Type coerce
            final_key = path[-1]
            if env_var in ("JARVIS_CHANNELS", "JARVIS_CHANNELS_ENABLED"):
...
def _dict_to_config(data: dict[str, Any]) -> JARVISConfig:
    """Convert a flat/nested dict to JARVISConfig with sub-dataclasses."""
    ...
    return JARVISConfig(
        **{k: v for k, v in data.items() if k in JARVISConfig.__dataclass_fields__},
        llm=LLMConfig(
...
def load_config(
    ...
    """
    Load JARVIS configuration with layered precedence:
    ...
    logger.info(
        "JARVIS config loaded: profile=%s, mode=%s, trust=%s, llm=%s",
        cfg.launch.profile,
        cfg.mode,
...
name: JARVIS
agent: jarvis             # Active agent (folder under ~/.prometheus/agents/)
