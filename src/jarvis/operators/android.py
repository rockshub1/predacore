"""
JARVIS Android device operator via ADB (Android Debug Bridge).
...
class AndroidOperator(BaseOperator):
    ...
    def __init__(
        ...
        self._serial = device_serial or os.getenv("JARVIS_ADB_SERIAL", "")
    ...
    def _ui_dump(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if not xml_content:
            ...
            remote_path = f"/sdcard/jarvis_ui_dump_{_uuid.uuid4().hex[:8]}.xml"
    ...
    def _screenshot(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if target:
            ...
            fd, temp_path = tempfile.mkstemp(prefix="jarvis_android_", suffix=".png")
            os.close(fd)
        ...
        if not captured:
            import uuid
            remote = f"/sdcard/jarvis_screenshot_{uuid.uuid4().hex}.png"
    ...
    def _screen_record(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        import uuid as _uuid
        remote_path = f"/sdcard/jarvis_recording_{_uuid.uuid4().hex[:8]}.mp4"
        local_path = str(params.get("local_path", "")).strip()
        if not local_path:
            local_path = os.path.expanduser("~/.prometheus/android_recording.mp4")
