"""
JARVIS Browser Bridge v2 — Full browser control at native speed.
...
_JARVIS_READ_PAGE_JS = r"""
(function _jarvis_read_page() {
...
class BrowserBridge:
    ...
    async def get_page_tree(self) -> dict[str, Any]:
        ...
        t0 = time.time()
        raw = await self._eval(_JARVIS_READ_PAGE_JS)
    ...
    async def screenshot(self, path: str = "", full_page: bool = False) -> dict[str, Any]:
        ...
        if self.is_cdp:
            ...
            if not path:
                fd, path = tempfile.mkstemp(prefix="jarvis_browser_", suffix=".png")
                os.close(fd)
    ...
    async def print_pdf(self, path: str = "") -> dict[str, Any]:
        ...
        if not path:
            fd, path = tempfile.mkstemp(prefix="jarvis_browser_", suffix=".pdf")
            os.close(fd)
        Path(path).write_bytes(data)
        ...
        if video_id:
            try:
                ...
                if shutil.which("yt-dlp"):
                    vtt_path = f"/tmp/jarvis_transcript_{video_id}"
    ...
    async def start_network_log(self) -> dict[str, Any]:
        ...
        await self._eval('''
            window.__jarvis_net_log = window.__jarvis_net_log || [];
            if (!window.__jarvis_net_observer) {
                window.__jarvis_net_observer = new PerformanceObserver(function(list) {
                    list.getEntries().forEach(function(entry) {
                        if (entry.entryType === "resource") {
                            window.__jarvis_net_log.push({
                            ...
                            // Cap at 500 entries
                            if (window.__jarvis_net_log.length > 500) window.__jarvis_net_log.shift();
                        }
                    });
                });
                window.__jarvis_net_observer.observe({entryTypes: ["resource"]});
            }
        ''')
    ...
    async def get_network_log(self, limit: int = 50) -> dict[str, Any]:
        ...
        result = await self._eval(f'''
            JSON.stringify({{
                requests: (window.__jarvis_net_log || []).slice(-{limit}),
                count: (window.__jarvis_net_log || []).length
            }})
        ''')
    ...
    async def clear_network_log(self) -> dict[str, Any]:
        ...
        await self._eval("window.__jarvis_net_log = [];")
        return {"ok": True}
    ...
    async def set_auth_credentials(self, username: str, password: str) -> dict[str, Any]:
        ...
        await self._eval(f'''
            ...
            window.__jarvis_auth = "Basic {auth_b64}";
        ''')
    ...
    async def clear_auth_credentials(self) -> dict[str, Any]:
        ...
        await self._eval("delete window.__jarvis_auth;")
        return {"ok": True}
    ...
    async def set_dialog_handler(self, mode: str = "accept", prompt_text: str = "") -> dict[str, Any]:
        ...
        By default, dialogs are auto-accepted on connect so JARVIS never hangs.
        """
        if not self.is_cdp:
    ...
    async def element_screenshot(self, selector: str, path: str = "") -> dict[str, Any]:
        ...
        if self.is_cdp:
            ...
            if not path:
                fd, path = tempfile.mkstemp(prefix="jarvis_element_", suffix=".png")
                os.close(fd)
    ...
    async def download_image(self, src: str = "", selector: str = "", path: str = "") -> dict[str, Any]:
        ...
        if not path:
            fd, path = tempfile.mkstemp(prefix="jarvis_img_", suffix=ext)
            os.close(fd)
        Path(path).write_bytes(img_bytes)
    ...
    async def capture_canvas(self, selector: str = "canvas", path: str = "") -> dict[str, Any]:
        ...
        img_bytes = base64.b64decode(b64_data)
        if not path:
            fd, path = tempfile.mkstemp(prefix="jarvis_canvas_", suffix=".png")
            os.close(fd)
        Path(path).write_bytes(img_bytes)
        ...
        if chrome_already_running:
            ...
            try:
                ...
                logger.warning(
                    ...
                    "Quit Chrome and let JARVIS relaunch it, or restart Chrome with: %s",
                    self.chrome_debug_command(self._cdp_port),
                )
                return False
            except (OSError, FileNotFoundError):
