"""
Unit tests for jarvis.llm_providers._anthropic_wire — shared Anthropic
wire format helpers used by anthropic.py and claude_dev.py.
...
from jarvis.llm_providers import _anthropic_wire as wire
...
class TestBuildSystemBlocks:
    ...
    def test_identity_prefix_without_user_system(self):
        """OAuth path with no user system — identity block gets the cache breakpoint."""
        result = wire.build_system_blocks(
            "", identity_prefix="You are Claude Code"
        )
        assert len(result) == 1
    ...
    def test_identity_prefix_with_user_system(self):
        """OAuth path with user system — identity first, user system second with cache."""
        result = wire.build_system_blocks(
            "You are JARVIS",
            identity_prefix="You are Claude Code",
        )
        ...
        assert result[1]["text"] == "You are JARVIS"
