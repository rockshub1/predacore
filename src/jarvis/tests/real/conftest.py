...
Credential sources:
  ...
  * ``CLAUDE_CODE_OAUTH_TOKEN`` env var OR macOS Keychain entry
    ``Claude Code-credentials`` — for ``test_claude_dev_real.py``
...
def _have_claude_code_oauth() -> bool:
    """True if a Claude Code OAuth token is reachable (env or keychain)."""
    if os.getenv("CLAUDE_CODE_OAUTH_TOKEN", "").strip():
        return True
    try:
        from jarvis.llm_providers.claude_dev import _load_keychain_token
...
@pytest.fixture
def claude_dev_provider():
    """ClaudeDevProvider using whatever OAuth token is reachable."""
    if not _have_claude_code_oauth():
        pytest.skip(
            "No Claude Code OAuth token reachable (neither "
            "CLAUDE_CODE_OAUTH_TOKEN env var nor macOS Keychain entry "
            "'Claude Code-credentials'). Run `claude` once to log in, then "
            "rerun with --real."
        )

    from jarvis.llm_providers.claude_dev import ClaudeDevProvider
    from jarvis.llm_providers.base import ProviderConfig

    return ClaudeDevProvider(
        ProviderConfig(
            model="claude-opus-4-6",
            max_tokens=4096,
            reasoning_effort="medium",
            extras={"provider": "claude-dev"},
        )
    )
