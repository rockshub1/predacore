...
Usage:
    ...
    python -m pytest jarvis/tests/test_android_real.py -v
"""
...
class TestClipboard:
    def test_set_and_get_clipboard(self, operator):
        result = operator.execute("set_clipboard", {"text": "JARVIS Android test"})
        assert result["ok"]
...
class TestShell:
    def test_shell_command(self, operator):
        result = operator.execute("shell", {"command": "echo hello_jarvis"})
        assert result["ok"]
        assert "hello_jarvis" in result["output"]
...
class TestActionCount:
    def test_total_actions(self, operator):
        from jarvis.operators.enums import AndroidAction
