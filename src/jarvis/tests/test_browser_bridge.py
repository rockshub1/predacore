...
Usage:
    ...
    python -m pytest jarvis/tests/test_browser_bridge.py -v --timeout=60
"""
...
# Set JARVIS_BROWSER_HEADED=1 to watch tests in a real Chrome window
HEADED = os.environ.get("JARVIS_BROWSER_HEADED", "").lower() in ("1", "true", "yes")
...
<html lang="en">
<head><meta charset="UTF-8"><title>JARVIS Test Page</title></head>
<body>
  <h1 id="heading">Hello JARVIS</h1>
  ...
  <button id="prompt-btn" onclick="document.getElementById('prompt-result').textContent=prompt('Enter name:','JARVIS')">Trigger Prompt</button>
...
def chrome_process():
    ...
    user_data = tempfile.mkdtemp(prefix="jarvis_chrome_test_")
    cmd = [
        CHROME_BIN,
...
class TestNavigation:
    ...
    async def test_navigate_to_page(self, bridge, test_server):
        ...
        assert result["ok"]
        assert "JARVIS Test Page" in result.get("title", "")
...
class TestType:
    ...
    async def test_type_inject(self, bridge):
        ...
        result = await bridge.type_text(selector="#name-input", value="JARVIS")
        assert result.get("ok")
...
class TestWaitPrimitives:
    ...
    async def test_wait_for_text(self, bridge):
        result = await bridge.wait_for_text("Hello JARVIS", timeout=3)
        assert result["found"]
...
class TestFindInPage:
    ...
    async def test_find_existing_text(self, bridge):
        result = await bridge.find_in_page("JARVIS")
        assert result.get("count", 0) > 0
        assert any("JARVIS" in m.get("text", "") for m in result.get("matches", []))
...
class TestClipboard:
    ...
    async def test_clipboard_write(self, bridge):
        result = await bridge.clipboard_write("JARVIS clipboard test 42")
...
class TestJSDialogs:
    ...
    async def test_alert_auto_accepted(self, bridge):
        """alert() should be auto-accepted — JARVIS should NOT hang."""
    ...
    async def test_prompt_auto_accepted(self, bridge):
        ...
        assert result == "JARVIS"  # prompt() returned the default value
