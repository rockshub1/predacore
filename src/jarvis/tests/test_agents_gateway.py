"""
Comprehensive tests for JARVIS orchestration layer — Phase 5.
...
from jarvis.agents.meta_cognition import (
    ResponseEvaluation,
    evaluate_response,
    detect_loop,
    should_ask_for_help,
...
from jarvis.agents.self_improvement import (
    ProposalCategory,
    RiskLevel,
    ImprovementProposal,
    AuditLog,
...
class TestGatewayRateLimiting:
    ...
    def _make_gateway(self):
        ...
        with patch("jarvis.gateway.SessionStore"), \
             patch("jarvis.gateway.LaneQueue"), \
             patch("jarvis.gateway.ChannelHealthMonitor"), \
             patch("jarvis.gateway.IdentityService"), \
             patch("jarvis.gateway.LRUCache"), \
             patch("jarvis.gateway.RateLimiter"), \
             patch("jarvis.gateway.default_api_limits", return_value=[]), \
             patch("jarvis.gateway.AuthMiddleware", return_value=None), \
             patch.dict(os.environ, {"JARVIS_JWT_SECRET": ""}, clear=False):
