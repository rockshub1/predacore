"""
Comprehensive tests for jarvis.config — 3-layer configuration system.
...
class TestJARVISConfigMaster:
    """Tests for the master JARVISConfig dataclass."""
    ...
    def test_custom_home_dir(self):
        cfg = JARVISConfig(home_dir="/tmp/custom_prom")
    ...
    def test_agents_dir_property(self):
        cfg = JARVISConfig(home_dir="/tmp/prom")
        assert cfg.agents_dir == "/tmp/prom/agents"

    def test_agent_dir_property(self):
        cfg = JARVISConfig(home_dir="/tmp/prom", agent="jarvis")
        assert cfg.agent_dir == "/tmp/prom/agents/jarvis"
...
class TestProfilePresets:
    ...
    def test_balanced_trust_normal(self):
        ...
        assert p["launch"]["max_tool_iterations"] == 10
    ...
    def test_public_beast_is_aggressive(self):
        ...
        assert p["launch"]["max_tool_iterations"] == 30
...
class TestEnvOverrides:
    ...
    def test_trust_level_override(self):
        with patch.dict(os.environ, {"JARVIS_TRUST_LEVEL": "paranoid"}, clear=True):
            result = _env_overrides()
            assert result.get("security", {}).get("trust_level") == "paranoid"

    def test_channels_csv(self):
        with patch.dict(os.environ, {"JARVIS_CHANNELS": "cli,telegram,discord"}, clear=True):
            result = _env_overrides()
            assert result.get("channels", {}).get("enabled") == ["cli", "telegram", "discord"]

    def test_bool_env_vars(self):
        with patch.dict(os.environ, {"JARVIS_ENABLE_SELF_EVOLUTION": "true"}, clear=True):
            result = _env_overrides()
            assert result.get("launch", {}).get("enable_self_evolution") is True

    def test_int_env_vars(self):
        with patch.dict(os.environ, {"JARVIS_MAX_TOOL_ITERATIONS": "25"}, clear=True):
            result = _env_overrides()
            assert result.get("launch", {}).get("max_tool_iterations") == 25

    def test_float_env_vars(self):
        with patch.dict(os.environ, {"JARVIS_PERSONA_DRIFT_THRESHOLD": "0.55"}, clear=True):
            result = _env_overrides()
...
class TestLoadConfig:
    ...
    def test_directories_created(self, tmp_path):
        ...
        with patch.dict(os.environ, {"PROMETHEUS_HOME": str(home)}, clear=False):
...
class TestModuleConstants:
    ...
    def test_default_home(self):
        assert DEFAULT_HOME == Path.home() / ".prometheus"

    def test_default_profile(self):
        assert DEFAULT_PROFILE == "balanced"

    def test_default_agent(self):
        assert DEFAULT_AGENT == "jarvis"
