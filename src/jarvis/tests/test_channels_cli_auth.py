"""
Comprehensive tests for JARVIS interface layer — Phase 6.
...
import jarvis.channels  # noqa: F401

from jarvis.channels.health import (
    ChannelStatus,
    RateLimiter,
    ReconnectionManager,
    ChannelHealthRecord,
...
from jarvis.channels.whatsapp import (
    WhatsAppAdapter,
...
from jarvis.channels.webchat import (
    WebChatAdapter,
    STATIC_DIR,
)
...
from jarvis.auth.sandbox import (
...
from jarvis.auth.security import (
    InjectionAssessment,
    detect_injection,
...
from jarvis.gateway import ChannelAdapter, IncomingMessage, OutgoingMessage
...
def _make_mock_config(**overrides):
    """Create a real JARVISConfig for adapter construction."""
    from jarvis.config import JARVISConfig, ChannelConfig
    ...
    config = JARVISConfig(
        name=overrides.get("name", "JARVIS"),
...
class TestTelegramAdapter:
    ...
    def test_is_bot_mentioned_direct():
        update = MagicMock()
        update.message = MagicMock()
        update.message.text = "Hey @jarvis_bot how are you?"
        update.message.entities = None
        context = MagicMock()
        context.bot.username = "jarvis_bot"
...
class TestSessionSandbox:
    ...
    def test_volume_name_format(self):
        ...
        assert sb.volume_name.startswith("jarvis-sandbox-")
    ...
    def test_volume_name_sanitized(self):
        ...
        assert "/" not in vname.replace("jarvis-sandbox-", "")
...
class TestSSRFProtection:
    ...
    def test_valid_https_url(self):
        ...
        with patch("jarvis.auth.security.socket.getaddrinfo") as mock_dns:
    ...
    def test_dns_rebinding_detection(self):
        ...
        with patch("jarvis.auth.security.socket.getaddrinfo", side_effect=mock_dns):
    ...
    def test_dns_resolution_failure(self):
        import socket
        with patch(
            "jarvis.auth.security.socket.getaddrinfo",
...
# ═══════════════════════════════════════════════════════════════════════
# 11. CLI — argument parsing, GenerationController, slash commands
# ═══════════════════════════════════════════════════════════════════════


class TestGenerationController:
    """GenerationController lifecycle."""

    def test_initial_state(self):
        gc = GenerationController()
        assert gc.cancelled is False
        assert gc._partial_response == ""
        assert gc._is_generating is False

    def test_cancel(self):
        gc = GenerationController()
        gc.cancel()
        assert gc.cancelled is True

    def test_accumulate(self):
        gc = GenerationController()
        gc.accumulate("Hello")
        gc.accumulate(" world")
        assert gc._partial_response == "Hello world"

    def test_reset(self):
        gc = GenerationController()
        gc.cancel()
        gc.accumulate("partial")
        gc._is_generating = True
        gc.reset()
        assert gc.cancelled is False
        assert gc._partial_response == ""
        assert gc._is_generating is False
...
class TestJARVISCompleter:
    ...
    def test_slash_completions(self):
        completer = JARVISCompleter()
        doc = MagicMock()
        doc.text_before_cursor = "/ex"
    ...
    def test_model_completions(self):
        ...
        completer = JARVISCompleter(get_models_fn=lambda: models)
        doc = MagicMock()
        doc.text_before_cursor = "/model gpt"
        completions = list(completer.get_completions(doc, None))
        values = [c.text for c in completions]
...
class TestStatusBar:
    ...
    def test_format_small_numbers(self):
        assert StatusBar._fmt(500) == "500"

    def test_format_thousands(self):
        result = StatusBar._fmt(5000)
        assert "k" in result

    def test_format_millions(self):
        result = StatusBar._fmt(2_500_000)
        assert "M" in result


class TestCLIArgParsing:
    """CLI main() argparse — 11 subcommands."""

    def test_all_subcommands_exist(self):
        """The 11 subcommands from docs should all be parseable."""
        import argparse
        parser = argparse.ArgumentParser()
        subparsers = parser.add_subparsers(dest="command")

        # These are the subcommands defined in main()
        expected = [
            "chat", "start", "stop", "restart", "install",
            "logs", "setup", "auth", "status", "eval", "doctor",
        ]
        for cmd in expected:
    ...
    def test_command_defs_completeness(self):
        """All slash commands in _COMMAND_DEFS are documented."""
        names = {name for name, _, _ in _COMMAND_DEFS}
        expected = {
            "/model", "/status", "/tools", "/sessions", "/new",
            "/retry", "/copy", "/cost", "/history", "/compact",
            "/clear", "/help", "/exit",
        }
...
class TestSlashCommands:
    """Verify slash command parsing and return values."""

    @pytest.mark.asyncio
    async def test_exit_command(self):
        from jarvis.cli import _handle_command
        result = await _handle_command("/exit", MagicMock(), MagicMock())
        assert result == "exit"

    @pytest.mark.asyncio
    async def test_quit_command(self):
        from jarvis.cli import _handle_command
        result = await _handle_command("/quit", MagicMock(), MagicMock())
        assert result == "exit"

    @pytest.mark.asyncio
    async def test_q_command(self):
        from jarvis.cli import _handle_command
        result = await _handle_command("/q", MagicMock(), MagicMock())
        assert result == "exit"

    @pytest.mark.asyncio
    async def test_retry_with_message(self):
        from jarvis.cli import _handle_command
        result = await _handle_command(
            "/retry", MagicMock(), MagicMock(),
            last_user_message="previous question",
        )
        assert result == "retry"

    @pytest.mark.asyncio
    async def test_retry_no_message(self):
        from jarvis.cli import _handle_command
        result = await _handle_command(
            "/retry", MagicMock(), MagicMock(),
            last_user_message="",
    ...
    async def test_new_session(self):
        from jarvis.cli import _handle_command
        sb = StatusBar()
        result = await _handle_command("/new", MagicMock(), MagicMock(), status_bar=sb)
        assert result is not None
        assert result.startswith("new_session:")

    @pytest.mark.asyncio
    async def test_compact_returns_resend(self):
        from jarvis.cli import _handle_command
        result = await _handle_command("/compact", MagicMock(), MagicMock())
        assert result is not None
        assert result.startswith("resend:")

    @pytest.mark.asyncio
    async def test_unknown_command(self):
        from jarvis.cli import _handle_command
        result = await _handle_command("/nonexistent", MagicMock(), MagicMock())
        assert result is None

    @pytest.mark.asyncio
    async def test_clear_command(self):
        from jarvis.cli import _handle_command
        result = await _handle_command("/clear", MagicMock(), MagicMock())
        assert result is None

    @pytest.mark.asyncio
    async def test_help_command(self):
        from jarvis.cli import _handle_command
        result = await _handle_command("/help", MagicMock(), MagicMock())
        assert result is None
