"""
JARVIS Core — The conversational agent brain.
...
class JARVISCore:
    ...
    def __init__(self, config: JARVISConfig):
        ...
        # Propagate config flags to env so MCP server subprocess can read them
        if config.launch.enable_openclaw_bridge:
            os.environ["JARVIS_ENABLE_OPENCLAW_BRIDGE"] = "1"
        if config.launch.enable_plugin_marketplace:
            os.environ["JARVIS_ENABLE_PLUGIN_MARKETPLACE"] = "1"
        ...
        # World model (JAX RSSM) — permanently disabled. The class still
        # exists in services/world_model.py for reference, but nothing
        # imports or instantiates it. All downstream paths short-circuit
        # on self._world_model is None.
        ...
        self._tool_timeout = int(os.getenv("JARVIS_TOOL_TIMEOUT_SECONDS", "1800"))

        logger.info(
            "JARVISCore initialized (trust=%s, tools=%d, provider=%s, max_tool_iterations=%d)",
    ...
    def _build_persona_anchor_terms(self) -> set[str]:
        ...
        anchors = {
            str(self.config.name or "JARVIS").strip().lower(),
            "jarvis",
            "prometheus",
            "project prometheus",
        }

        env_raw = os.getenv("JARVIS_PERSONA_ANCHORS", "")
        if env_raw:
    ...
    def _persona_regeneration_instruction(
        ...
        base = (
            ...
            f"- Speak as {self.config.name} from Project Prometheus.\n"
    ...
    async def _build_runtime_memory_context(self, user_id: str, query: str) -> str:
        ...
        try:
            top_k = int(os.getenv("JARVIS_MEMORY_PROMPT_TOP_K", "5"))
        except (ValueError, TypeError):
            top_k = 5
        top_k = max(1, min(top_k, 20))

        try:
            max_chars = int(os.getenv("JARVIS_MEMORY_PROMPT_MAX_CHARS", "1800"))
        except (ValueError, TypeError):
            max_chars = 1800
    ...
    async def _append_decision_journal(
        ...
        *,
        ...
        try:
            ...
            if not engine.seed_exists or not engine.is_bootstrapped:
                return
    ...
    async def process(
        ...
        # During bootstrap, use a focused prompt — no runtime context noise
        from .identity.engine import get_identity_engine as _get_engine
        _id_engine = _get_engine(self.config)
        bootstrap_mode = _id_engine.needs_bootstrap
        if bootstrap_mode:
            # First contact — use the full identity prompt (SOUL_SEED + EVENT_HORIZON
            # + BOOTSTRAP.md) via build_identity_prompt(), so the Event Horizon
            # protocol is loaded alongside the bootstrap conversation guide.
            # Plus explicit tool instructions so the model saves identity files.
            identity_prompt = _id_engine.build_identity_prompt()
            tool_instructions = (
                "\n\n---\n\n"
                "## CRITICAL: You MUST save your identity with these tools\n\n"
                "After the bootstrap conversation, call `identity_update` for "
                "ALL 7 files (in any order). Pass the bare name without `.md`:\n"
                "1. `identity_update` file=\"IDENTITY\" — your name, nature, vibe, emoji\n"
                "2. `identity_update` file=\"SOUL\" — your personality, values, style\n"
                "3. `identity_update` file=\"USER\" — what you learned about your human\n"
                "4. `identity_update` file=\"MEMORY\" — durable facts to carry forward\n"
                "5. `identity_update` file=\"TOOLS\" — verified local environment notes\n"
                "6. `identity_update` file=\"HEARTBEAT\" — your background discipline policy\n"
                "7. `identity_update` file=\"REFLECTION\" — your self-correction rules\n\n"
                "Then call `bootstrap_complete` to finalize. ALL 7 files must exist "
                "or bootstrap_complete will fail. Without this, you will forget who "
                "you are on restart."
            )
            system_prompt = f"{identity_prompt}{tool_instructions}"
            logger.info(
                "Bootstrap mode: full identity prompt + tool instructions (%d chars)",
                len(system_prompt),
            )
        else:
            # Pull relevant long-term memory snippets for this turn
            system_prompt = self._system_prompt

        # Skip memory retrieval during bootstrap — first contact should be clean.
        memory_context = ""
        ...
        # Bootstrap is first-contact by definition — prior session history would
        # drown out the bootstrap prompt (e.g. 33k tokens of task work vs. 3k
        # tokens of "you're just waking up, just talk"). Skip history entirely.
        if not bootstrap_mode:
            messages.extend(
        ...
        _mcp_tool_log: list[dict] = []  # MCP tool execution log for metrics
        ...
        for iteration in range(self._max_tool_iterations):
            ...
            try:
                ...
                _mcp_provider = self.llm.executes_tool_loop_internally
                _stream_this_iter = (
                    stream_fn if (not _tools_this_iter or _mcp_provider) else None
                )
            ...
            # MCP providers execute tools internally — capture for metrics
            mcp_tools = response.get("tools_executed", [])
            if mcp_tools:
                for mt in mcp_tools:
                    _tools_used.append(mt["name"])
                    _mcp_tool_log.append(mt)
                    executed_tool_calls += 1
                logger.info(
                    "MCP tools executed: %s", ", ".join(t["name"] for t in mcp_tools)
                )
            ...
            #
            #   structured_path (Anthropic/claude_dev): the provider
