# JARVIS Vendor Layer Documentation

Comprehensive reference for all vendored dependencies under `src/jarvis/_vendor/`. These modules originate from the earlier Prometheus microservice architecture and are vendored (copied into the JARVIS source tree) rather than pip-installed so that JARVIS can operate as a single-process monolith without requiring gRPC inter-service calls, Docker orchestration, or network-accessible microservices. Import paths are rewritten from `src.common.*` to `jarvis._vendor.common.*` etc.
...
2. [Common Utilities (`common/`)](#2-common-utilities)
   ...
   - [skill_hivemind.py](#214-skill_hivemindpy)
...
Empty package initializer. Makes `jarvis._vendor` importable as a Python package. Contains no logic.

**Why vendored:** The original Prometheus platform ran as 5+ microservices communicating over gRPC. JARVIS collapsed these into a single process; the vendored code preserves the data models, proto stubs, and service logic without requiring live gRPC servers.
...
Shared foundation used by every other vendor subsystem and by JARVIS core. Provides embedding clients, LLM provider abstraction, memory persistence, metrics, error hierarchy, logging, notification delivery, data models, vector search, and the Flame skill network primitives.
...
**Purpose:** Pydantic data models shared across all Prometheus components. Defines the domain types for Knowledge Nexus nodes/edges, CSC plans/steps, WIL tool descriptions/interactions, and DAF agent/task assignments.
...
|-----------|------|---------|-------------|
| `error_code` | `str` | `"PROMETHEUS_ERROR"` | Machine-readable identifier |
...
Emits single-line JSON: `{"ts": ISO8601, "level": "INFO", "logger": "jarvis.core", "msg": "...", "trace_id": "...", ...}`. Adds `file:line` for WARNING+. Includes exception info. Passes through all non-standard LogRecord attributes as extra fields.

#### Class: `PrettyFormatter(logging.Formatter)`

Colored terminal output: `02:30:00 > INFO jarvis.core Processing message [trace:abc12345]`. Uses ANSI color codes: cyan/green/yellow/red/magenta for DEBUG/INFO/WARNING/ERROR/CRITICAL.
...
- `sign(secret=None)`: HMAC-SHA256 over deterministic JSON of id, name, version, creator, steps, declared_tools, tier. Uses `JARVIS_SKILL_SIGNING_SECRET` env var or machine-local derived key
...
**Constructor:** Takes `instance_id`, `local_dir` (~/.prometheus/flame/local/), `shared_dir` (~/.prometheus/flame/shared/).
...
- Local skills: `~/.prometheus/flame/local/{genome_id}.json`
- Shared skills: `~/.prometheus/flame/shared/{genome_id}.json`
- Reputation: `~/.prometheus/flame/shared/reputation.json`
...
**Persistence:** Saves to `~/.prometheus/skills/evolved/evolution_state.json` containing crystallized genomes, pending endorsements, and pattern data.
...
The strategic planning subsystem. Originally a separate gRPC microservice, now vendored for JARVIS's integrated planning pipeline. Provides HTN (Hierarchical Task Network) planning, LLM-driven planning, and MCTS (Monte-Carlo Tree Search) plan optimization.

**`__init__.py`:** Single docstring: `"Vendored Core Strategic Engine -- only planner_mcts needed by JARVIS."`
...
Safety and compliance layer that gates agent actions. Originally a gRPC microservice; now vendored for JARVIS's integrated governance pipeline.

**`__init__.py`:** `"Vendored Ethical Governance Module -- only persistent_audit needed by JARVIS."`
...
common/skill_genome.py --> common/skill_scanner.py (SkillGenome, CapabilityTier, TrustLevel, etc.)
                    -----> common/skill_hivemind.py (SkillGenome, TrustLevel, TIER_PROPAGATION)
                    -----> common/skill_evolution.py (SkillGenome, SkillStep, CapabilityTier)

common/skill_scanner.py -> common/skill_hivemind.py (ScanVerdict, SkillScanner)
...
### How JARVIS core imports vendor modules

JARVIS's main codebase imports:
- `jarvis._vendor.common.embedding` -- for `get_default_embedding_client()`
- `jarvis._vendor.common.llm` -- for `get_default_llm_client()`
- `jarvis._vendor.common.memory_service` -- for `MemoryService`
- `jarvis._vendor.common.errors` -- for the error hierarchy
- `jarvis._vendor.common.metrics` -- for observability
- `jarvis._vendor.common.logging_config` -- for `setup_logging()`
- `jarvis._vendor.common.models` -- for Plan, PlanStep, StatusEnum
- `jarvis._vendor.common.skill_genome` -- for SkillGenome
- `jarvis._vendor.common.skill_scanner` -- for SkillScanner
- `jarvis._vendor.common.skill_hivemind` -- for Flame
- `jarvis._vendor.common.skill_evolution` -- for SkillCrystallizer
- `jarvis._vendor.core_strategic_engine.planner_mcts` -- for ABMCTSPlanner
- `jarvis._vendor.ethical_governance_module.persistent_audit` -- for PersistentAuditStore
- `jarvis._vendor.knowledge_nexus` -- for KnowledgeNexusService
- `jarvis._vendor.user_modeling_engine` -- for UserModelingEngineService, UserProfile
...
*This documentation covers every source file, every class, every method, every protobuf message, and every algorithm in the JARVIS vendor layer. Total files documented: 44 source files across 5 subsystems.*
