# JARVIS Infrastructure Layer Documentation

> Exhaustive reference for the five infrastructure folders that power JARVIS:
...
**Path:** `src/jarvis/services/`
**Files:** 19 source files, ~334 KB
**Purpose:** The services layer is the operational backbone of JARVIS. It provides the daemon lifecycle, database infrastructure, task scheduling, rate limiting, alerting, plugin system, voice interface, code indexing, git integration, outcome tracking, session-serialized execution, transcript persistence, config hot-reload, embedding clients, identity management, and a JAX/Flax world model.
...
### 1.1 `daemon.py` -- JARVIS Background Daemon

**Purpose:** Manages the full JARVIS lifecycle as a background process: PID file management, signal handling, health endpoints, auto-restart, heartbeat, and graceful shutdown.
...
| `daemon` | `JARVISDaemon` | Reference to parent daemon for status data |
...
The main daemon class. Orchestrates the entire JARVIS runtime.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `config` | `JARVISConfig` | Full JARVIS configuration |

**Instance Variables:**

| Variable | Type | Purpose |
|----------|------|---------|
| `config` | `JARVISConfig` | Configuration reference |
...
| `_core` | `JARVISCore | None` | Brain instance |
...
Generates a macOS launchd plist XML for auto-starting JARVIS at login.

##### `install_launchd(config) -> Path`
Writes the plist to `~/Library/LaunchAgents/com.prometheus.jarvis.plist`.

---

### 1.2 `db_server.py` -- Unix Domain Socket Database Service

**Purpose:** Provides single-writer, concurrent-reader SQLite access over a Unix domain socket using a length-prefixed JSON-RPC protocol. This centralizes all database access through one process, eliminating SQLite locking conflicts when daemon, MCP subprocess, and tools all need DB access.
...
| `socket_path` | `str | None` | Unix socket path (default: `~/.prometheus/db.sock`) |
...
##### `EmailDispatcher`
- SMTP with TLS. Configurable via env vars (`JARVIS_ALERT_SMTP_*`).
...
**Purpose:** Tick-based cron scheduler that reads job definitions from `~/.prometheus/cron.yaml` and executes them through the Gateway.
...
**Purpose:** Adaptive neural model (adapted from DreamerV3) that learns from JARVIS's experience. Projects GTE-small 384-dim embeddings to 128-dim task-adapted embeddings.
...
**Path:** `src/jarvis/memory/`
**Files:** 4 source files (Python orchestration) + `jarvis_core_crate/` (Rust compute kernel)
**Purpose:** Unified memory system. Python owns the schema, async orchestration, and scope filtering. Rust (`jarvis_core`) owns all hot compute: vector search, BM25, fuzzy matching, embeddings, entity extraction, relation classification.

**jarvis_core is a HARD dependency.** No Python fallbacks. No numpy cosine fallback. No Python BM25. No Python entity heuristic. If the Rust kernel fails, the memory system fails loudly — this is a deliberate design choice for correctness and predictable performance.
...
- **Search is Rust-only:** delegates to `jarvis_core.vector_search` (SIMD cosine). No numpy fallback. Raises on kernel failure.
...
- `async store(content, memory_type, importance, ...)` -- Generates embedding via `jarvis_core.embed` through the configured client, packs to BLOB, inserts into SQLite, adds to in-RAM vector cache. Atomic ordering: DB first, then vector index (DB is the source of truth; the cache can always be rebuilt).
...
- `_recall_keyword()` -- Rust BM25 via `jarvis_core.bm25_search`, after Rust synonym expansion via `jarvis_core.expand_synonyms`. Filters by memory_type and scope BEFORE ranking to keep the corpus small.
...
3. **Semantic search:** 1200 tokens -- vector similarity via `jarvis_core.vector_search`, reranked by `0.6*similarity + 0.25*recency + 0.15*importance`. Cached 60 s.
4. **Fuzzy matches:** 400 tokens -- trigram typo-tolerant via `jarvis_core.fuzzy_search`. Catches `"congif"` → `"config"` that semantic misses.
...
2. **`extract_entities_from_recent(limit=50)`** -- **Rust-first** via `jarvis_core.extract_entities` (3-tier: 100+ entity dictionary, regex patterns, stopword list). Optional LLM enrichment on longer text (`len >= 80`) adds novel entities not in the dictionary. LLM failures during enrichment are non-fatal — Rust results are still returned. Prompt injection defense: content wrapped in `<conversation_data>` tags.

3. **`auto_link()`** -- Finds entity pairs that co-occur 2+ times in the same memory. For each new pair, calls `jarvis_core.classify_relation(sentence, entity_a, entity_b)` for sentence-aware relation typing (window-based verb-phrase matching). If Rust confidence < 0.5 (no verb phrase matched), falls back to type-pair inference: person+tool → `uses`, person+project → `works_on`, etc. Skips pairs with existing relations (no weight inflation).
...
### 2.4 Rust Compute Kernel — `jarvis_core_crate/`

**Separate Rust crate** built via maturin. Python extension module `jarvis_core._core` (~10 MB `.so`). Mandatory dependency of `jarvis.memory` — no Python fallbacks.
...
claude_code.py -- Claude Code SDK provider
claude_harness.py -- Claude subprocess harness
...
**Path:** `src/jarvis/auth/`
**Files:** 5 source files, ~84 KB
**Purpose:** OAuth authentication, sandbox isolation, security filters, and HTTP middleware.

---

### 4.1 `oauth.py` -- Antigravity OAuth

**Purpose:** OAuth2 authentication with the Antigravity platform using PKCE-protected Authorization Code flow.

#### Credential Encryption

- `_derive_machine_key()` -- PBKDF2-HMAC-SHA256 with 480,000 iterations. Seed: `{hostname}-{login}-{home}`. Application-specific salt. Returns Fernet key.
- `_encrypt_value(value)` / `_decrypt_value(token)` -- Fernet encryption using machine-derived key. Credentials are only decryptable on the same machine.

#### Class: `OAuthToken`
Dataclass: `access_token`, `refresh_token`, `token_type`, `expires_at`, `scopes`, `created_at`. `is_expired` checks with 5-minute buffer. Serializable to/from dict.

#### Class: `TokenStore`

Disk persistence at `~/.prometheus/auth.json` with encryption.

- `save(token)` -- Encrypts sensitive fields (access_token, refresh_token), writes atomically (temp file + rename), sets file mode 0600.
- `load()` -- Verifies file permissions, tightens if too open, decrypts sensitive fields.

#### Class: `AntigravityAuth`

**OAuth2 Flow:**
1. Generate PKCE: `code_verifier` (64 bytes urlsafe) + SHA256 `code_challenge`.
2. Build auth URL with client_id, redirect_uri, scopes, state, PKCE.
3. Open browser.
4. `_listen_for_callback()` -- Local HTTP server on port 9876. Validates state parameter. Timeout: 120 seconds.
5. `_exchange_code()` -- POST to token endpoint with code + verifier.

**Token Management:**
- `refresh()` -- Async lock prevents concurrent refresh races. Double-checks token validity after acquiring lock.
- `ensure_valid_token()` -- Safe for concurrent coroutines.
- `login_with_token(token)` -- Direct token injection for CI/manual setup.
- Environment variable `ANTIGRAVITY_TOKEN` auto-populates.

#### Class: `AntigravityClient`

REST API client with automatic token management. Endpoints:
- Conversations: list, get, search
- Agent invocation: invoke, list agents
- Shared memory: store, recall
- User profile: get
- Platform tools: list, execute
- Self-improvement: submit feedback, get insights

---

### 4.2 `sandbox.py` -- Code Execution Isolation
...
Per-session ephemeral containers with named Docker volumes for state persistence across executions. Volume naming: `jarvis-sandbox-{safe_session_id}`.
...
### 4.3 `security.py` -- Prompt Injection Defense
...
### 4.4 `middleware.py` -- HTTP Auth Middleware
...
**Path:** `src/jarvis/channels/`
**Files:** 12 files (6 Python + 6 static web assets), ~181 KB
**Purpose:** Multi-platform message routing. Each adapter implements `ChannelAdapter` (from gateway.py) and connects JARVIS to a specific messaging platform.
...
**Purpose:** Connects JARVIS to Telegram via long-polling Bot API.
...
- `LLMRouter` (from llm_providers) is instantiated by `JARVISCore`.
...
- `JARVISBrain`: threading.Lock for thread-safe parameter access.
...
*End of JARVIS Infrastructure Layer Documentation.*
