"""
JARVIS ↔ DAF Bridge — Connects JARVIS orchestration to DAF process management.

Allows JARVIS multi-agent tasks to optionally dispatch to DAF for:
...
When DAF is unavailable, falls back to JARVIS's in-process async agents.
"""
...
class DAFBridge:
    """Bridge between JARVIS tool system and DAF process fabric.
    ...
    async def dispatch_multi_agent(
        ...
        *,
        ...
        try:
            import grpc
            from jarvis._vendor.common.protos import daf_pb2, daf_pb2_grpc, wil_pb2
        ...
        # Map JARVIS roles to DAF agent types (extensible via config)
        role_to_type = {
