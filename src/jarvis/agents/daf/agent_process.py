...
from jarvis._vendor.common.protos import daf_pb2, daf_pb2_grpc, wil_pb2, wil_pb2_grpc
...
class AgentProcess:
    ...
    def _setup_unified_memory(self):
        ...
        try:
            try:
                ...
                from src.jarvis.memory.store import UnifiedMemoryStore  # type: ignore

            prometheus_home = Path(
                os.getenv("PROMETHEUS_HOME")
                or os.getenv("JARVIS_HOME")
                or (Path.home() / ".prometheus")
            )
            db_path = prometheus_home / "memory" / "unified_memory.db"
    ...
    def _resolve_home_dir(self, context_data: dict[str, Any], params: dict[str, Any]) -> str:
        ...
        if explicit:
            return explicit
        return str(Path.home() / ".prometheus")

    def _ensure_rich_runtime(self, home_dir: str) -> bool:
        """Bootstrap the same direct-provider collaboration runtime used by JARVIS."""
        ...
        try:
            try:
                ...
                from jarvis.tools.subsystem_init import SubsystemFactory
                from jarvis.tools.handlers import HANDLER_MAP, ToolContext
                from jarvis.agents.engine import AgentEngine
            except ImportError:
                from src.jarvis.config import load_config  # type: ignore
                from src.jarvis.tools.subsystem_init import SubsystemFactory  # type: ignore
                from src.jarvis.tools.handlers import HANDLER_MAP, ToolContext  # type: ignore
                from src.jarvis.agents.engine import AgentEngine  # type: ignore
        ...
        try:
            self._run_async(
                self._unified_memory.store(
                    ...
                    source="jarvis.daf.agent_process",
        ...
        *,
        ...
        try:
            try:
                ...
                from src.jarvis.agents.engine import (  # type: ignore
                    compile_dynamic_agent_prompt,
                    create_dynamic_agent_spec,
                )
