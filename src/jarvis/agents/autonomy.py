"""
Autonomy runtime for OpenClaw delegation in JARVIS.
...
class OpenClawRuntimeSettings:
    ...
    @classmethod
    def from_config(cls, config: JARVISConfig) -> OpenClawRuntimeSettings:
        ...
        return cls(
            ...
            ledger_path=os.getenv("JARVIS_ACTION_LEDGER_PATH", default_ledger),
            idempotency_db_path=os.getenv(
                "JARVIS_IDEMPOTENCY_DB_PATH", default_idempotency
            ),
        )
...
class OpenClawBridgeRuntime:
    ...
    def __init__(self, config: JARVISConfig, db_adapter: Any = None):
        self._config = config
        self._db_adapter = db_adapter
        ...
        *,
        ...
        if self.kill_switch_active():
            raise KillSwitchActivatedError(
                "JARVIS kill switch is active (set JARVIS_KILL_SWITCH=0 to resume)"
            )
    ...
    def kill_switch_active(self) -> bool:
        ...
        Kill switch is active when:
          - JARVIS_KILL_SWITCH is truthy, or
          - JARVIS_KILL_SWITCH points to a file with a truthy value.
        """
        raw = os.getenv("JARVIS_KILL_SWITCH", "").strip()
        if not raw:
            return False
        if _parse_bool(raw):
            return True
