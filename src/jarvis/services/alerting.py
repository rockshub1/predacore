"""
Alerting — PagerDuty, Slack, Email, Discord, and webhook alerting for JARVIS.
...
logger = logging.getLogger("jarvis.services.alerting")
...
class PagerDutyDispatcher:
    ...
    def send(self, alert: Alert) -> bool:
        ...
        payload = {
            ...
            "dedup_key": alert.dedup_key or f"jarvis-{alert.title}",
            "payload": {
...
class EmailDispatcher:
    ...
    def __init__(
        ...
        self.smtp_host = smtp_host or os.getenv("JARVIS_ALERT_SMTP_HOST", "")
        self.smtp_port = int(os.getenv("JARVIS_ALERT_SMTP_PORT", str(smtp_port)))
        self.from_addr = from_addr or os.getenv("JARVIS_ALERT_SMTP_FROM", "")
        to_env = os.getenv("JARVIS_ALERT_SMTP_TO", "")
        self.to_addrs = to_addrs or ([a.strip() for a in to_env.split(",") if a.strip()] if to_env else [])
        self.username = username or os.getenv("JARVIS_ALERT_SMTP_USER", "")
        self.password = password or os.getenv("JARVIS_ALERT_SMTP_PASS", "")
...
class DiscordDispatcher:
    ...
    def __init__(self, webhook_url: str = "") -> None:
        self.webhook_url = webhook_url or os.getenv("JARVIS_ALERT_DISCORD_URL", "")
    ...
    def send(self, alert: Alert) -> bool:
        ...
        embed = {
            ...
            "color": color,
            "footer": {"text": f"JARVIS | {alert.source}"},
    ...
    def __init__(
        ...
        self._dispatchers: dict[AlertChannel, Any] = {
            ...
            AlertChannel.EMAIL: EmailDispatcher(
                smtp_host=smtp_host or os.getenv("JARVIS_ALERT_SMTP_HOST", ""),
                from_addr=smtp_from or os.getenv("JARVIS_ALERT_SMTP_FROM", ""),
                to_addrs=to_addrs,
            ),
            AlertChannel.DISCORD: DiscordDispatcher(
                webhook_url=discord_url or os.getenv("JARVIS_ALERT_DISCORD_URL", ""),
            ),
        }
