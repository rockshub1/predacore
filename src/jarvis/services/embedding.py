...
class RustEmbeddingClient(EmbeddingClient):
    """
    Rust-backed embedding via jarvis_core (BGE-small-en-v1.5 on Candle, 384-dim).

    Runs entirely offline, zero PyTorch dependency. The underlying model
    (BAAI/bge-small-en-v1.5, MTEB 62.2) is loaded once by jarvis_core at
    ...
    def _load(self) -> None:
        if self._module is None:
            ...
            self._module = jarvis_core
            logger.info(
                "RustEmbeddingClient: jarvis_core loaded (dim=%d)",
                jarvis_core.embedding_dim(),
            )
    ...
    async def embed(self, texts: list[str]) -> list[list[float]]:
        ...
        if not texts:
            return []
        self._load()
        # jarvis_core.embed releases the GIL internally; still offload to a
...
class LocalEmbeddingClient(EmbeddingClient):
    ...
    Kept as a PyTorch-based fallback for environments where jarvis_core is
...
def _rust_embedding_available() -> bool:
    """Check if jarvis_core (Rust embedder) is importable."""
    try:
        import jarvis_core  # noqa: F401
...
def get_default_embedding_client() -> EmbeddingClient:
    ...
    if provider == "rust":
        if not _rust_embedding_available():
            raise RuntimeError(
                "EMBED_PROVIDER=rust but jarvis_core is not importable. "
                "Build it via `maturin develop --release` in src/jarvis_core/."
            )
        return ResilientEmbeddingClient(
            primary=RustEmbeddingClient(), fallback=fallback
        )
