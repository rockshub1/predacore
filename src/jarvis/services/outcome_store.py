"""
JARVIS Outcome Store — Task outcome tracking with SQLite backend.
...
class OutcomeStore:
    ...
    def _get_conn(self) -> sqlite3.Connection:
        ...
        if conn is None:
            ...
            conn = sqlite3.connect(str(self._db_path), timeout=_timeout)
            conn.row_factory = sqlite3.Row
            conn.execute("PRAGMA journal_mode=WAL")
            conn.execute("PRAGMA synchronous=NORMAL")
            conn.execute(f"PRAGMA busy_timeout={_busy}")
