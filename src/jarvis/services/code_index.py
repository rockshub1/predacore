...
# ---------------------------------------------------------------------------
# Rust Embedder Adapter (wraps jarvis_core.embed for async interface)
# ---------------------------------------------------------------------------


class _RustEmbedder:
    """Wraps jarvis_core.embed() with the same async interface as EmbeddingClient."""

    async def embed(self, texts: list[str]) -> list[list[float]]:
        import jarvis_core
        import asyncio
        return await asyncio.to_thread(jarvis_core.embed, texts)
...
class CodeIndex:
    ...
    def __init__(self, repo_root: str | None = None, home_dir: str | None = None):
        ...
        self._home_dir = home_dir or os.path.expanduser("~/.prometheus")
    ...
    async def _ensure_embedder(self):
        if self._embedder is not None:
            return
        # Prefer Rust embedder (jarvis_core) — no PyTorch needed
        try:
            import jarvis_core
            self._embedder = _RustEmbedder()
            logger.info("Code index using Rust embedder (jarvis_core)")
            return
        except ImportError:
            pass
        # Fallback to Python embedder
        from jarvis._vendor.common.embedding import get_default_embedding_client
