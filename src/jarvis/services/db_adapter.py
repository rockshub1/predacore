"""
JARVIS DB Adapter — transparent socket-or-local SQLite access.

Provides a single :class:`DBAdapter` that tries to reach the running
:class:`~jarvis.services.db_server.DBServer` over its Unix socket.  If
...
Usage:
    adapter = DBAdapter(
        socket_path="~/.prometheus/db.sock",
...
_DEFAULT_SOCKET = str(Path.home() / ".prometheus" / "db.sock")
...
class DBAdapter:
    ...
    def __init__(
        ...
        self._socket_path: str = (
            os.environ.get("JARVIS_DB_SOCKET")
            or socket_path
            or _DEFAULT_SOCKET
        )
