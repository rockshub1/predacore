"""
Prometheus SDK — JARVIS's in-house LLM SDK.
...
httpx was chosen over aiohttp because:
  ...
  * Already the standard across JARVIS (handlers, vendored common/llm.py,
...
def make_client(*, timeout: float = 600.0, **kwargs: Any) -> httpx.AsyncClient:
    """Create an ``httpx.AsyncClient`` with JARVIS-standard defaults.

    600s timeout matches ``claude_dev.py`` — adaptive thinking on Opus 4.6
    can spend minutes in thinking before emitting any visible tokens, and
    cutting that off produces false failures.
    """
