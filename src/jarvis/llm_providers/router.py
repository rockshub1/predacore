"""
JARVIS LLM Router — Multi-provider LLM interface with failover.
...
# Import providers (direct API only — no SDK dependencies)
from .anthropic import AnthropicProvider
from .claude_dev import ClaudeDevProvider  # DEV ONLY — do not enable in public config
...
class LLMInterface:
    ...
    def __init__(self, config: JARVISConfig):
        self.config = config
        self._provider_name = config.llm.provider
        ...
        if config.llm.fallback_providers:
            for fb in config.llm.fallback_providers:
                if fb not in self._provider_chain:
                    self._provider_chain.append(fb)
    ...
    def _get_provider_instance(self, name: str) -> LLMProvider:
        ...
        elif name == "claude-dev":
            instance = ClaudeDevProvider(p_config)
