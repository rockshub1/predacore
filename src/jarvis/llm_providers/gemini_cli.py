...
Uses text-based tool calling: JARVIS injects tool definitions into the prompt
and parses <tool_call> blocks from the response. JARVIS controls the full
tool loop — no external MCP dependency.
...
class GeminiCLIProvider(LLMProvider):
    ...
    Tools are injected as text in the prompt. JARVIS parses <tool_call>
    blocks from the response and handles the tool loop itself — full control,
    no external MCP server needed.
    """

    name = "gemini-cli"
    supports_native_tools = False  # Text-based tool calling — JARVIS controls the loop
    ...
    async def chat(
        ...
        # Construct command
        _trust = os.getenv("JARVIS_TRUST_LEVEL", "normal").lower()
        ...
        # Parse tool calls from text response (JARVIS controls the loop)
