...
The round-trip bug in ``claude_dev.py`` / ``anthropic.py`` dropped
thinking block signatures and tool_use ids between iterations, and
``core.py`` substituted synthetic ``[Calling tool: X]`` text stubs for
real content blocks. After that was fixed, this validator remains as a
structural safety net so the same class of bug can never silently
re-enter the codebase without either tripping an Anthropic 400 or a
visible warning in the logs.
"""
