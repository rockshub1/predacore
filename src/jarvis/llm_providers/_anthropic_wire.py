...
Both ``anthropic.py`` (API-key auth via console.anthropic.com) and
``claude_dev.py`` (OAuth via Claude Code keychain) hit the same
``/v1/messages`` endpoint with the same body shape. This module owns the
stateless wire logic:
...
Each caller owns its own auth / headers / endpoint concerns. This module
is imported by both providers and has no state.
"""
...
def build_system_blocks(
    ...
    *,
    ...
    When ``identity_prefix`` is given (OAuth / Claude Code path), it goes
    into the first system block — Anthropic's OAuth enforcement requires
    it there. The user-provided system prompt goes into a second block
    with a cache breakpoint. For the API-key path, ``identity_prefix`` is
    None and the user system prompt is the only block, still with the
    cache breakpoint.
