"""
JARVIS Unified Memory Store — the single source of truth.

SQLite (schema + storage + scope filtering) layered over a Rust compute kernel
(jarvis_core: vector search, BM25, fuzzy, embeddings, entity extraction).

jarvis_core is a HARD dependency. No Python fallbacks. No numpy fallback.
...
class _NumpyVectorIndex:
    """
    In-RAM vector index backed by jarvis_core Rust SIMD cosine search.
    ...
    No disk persistence. No numpy fallback. jarvis_core is mandatory.
    """
    ...
    async def search(
        ...
        """
        Search top-k similar vectors via jarvis_core SIMD cosine.

        Raises RuntimeError if jarvis_core.vector_search fails — no fallback.
        """
        with self._lock:
            if not self._ids:
                return []
            ...
            raw_results = jarvis_core.vector_search(query_vector, self._vecs, fetch_k)
...
class UnifiedMemoryStore:
    """
    Single SQLite + vector index store for all JARVIS memory.
    ...
    def _open_db(self) -> sqlite3.Connection | None:
        if self._db_adapter is not None:
            return None
        # MCP subprocess needs longer timeout — daemon holds write lock
        _timeout = 30 if os.environ.get("JARVIS_MCP_SUBPROCESS") else 10
        _busy = 30000 if os.environ.get("JARVIS_MCP_SUBPROCESS") else 5000
        conn = sqlite3.connect(str(self._db_path), timeout=_timeout, check_same_thread=False)
        ...
        conn.execute(f"PRAGMA busy_timeout={_busy}")      # wait up to N ms on contention
    ...
    async def recall(
        ...
        """
        Recall memories by semantic similarity (jarvis_core SIMD cosine).
    ...
    async def _recall_keyword(
        ...
        """
        BM25 keyword search using jarvis_core (Rust). Used for empty queries
        ...
        if raw_terms:
            expanded = jarvis_core.expand_synonyms(raw_terms)
        ...
        ranked = jarvis_core.bm25_search(expanded_query, contents, top_k)
