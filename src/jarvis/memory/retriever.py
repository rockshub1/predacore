"""
JARVIS Memory Retriever — smart multi-source retrieval with context budgeting.
...
3. Semantic search   — vector similarity via jarvis_core SIMD (1200)
4. Fuzzy matches     — trigram typo-tolerant via jarvis_core (400)
5. Recent episodes   — session summaries for temporal continuity (remaining)

jarvis_core (Rust) is a HARD dependency — no Python fallbacks.
"""
...
class MemoryRetriever:
    ...
    async def _build_fuzzy_section(
        ...
        """Fuzzy search for typo-tolerant matching via jarvis_core (Rust trigrams)."""
        ...
        results = jarvis_core.fuzzy_search(query, contents, top_k=5, threshold=0.2)
        if not results:
            return ""
