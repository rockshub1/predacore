"""
JARVIS Marketplace Integration — OpenClaw skill import + executor wiring.
...
try:
    from jarvis.tools.skill_marketplace import (
        SkillCategory,
        SkillDefinition,
        SkillExecutor,
        SkillMarketplace,
    ...
    from src.jarvis.tools.skill_marketplace import (  # type: ignore
        SkillCategory,
        SkillDefinition,
        SkillExecutor,
        SkillMarketplace,
...
class MarketplaceManager:
    ...
    def __init__(self, config: "JARVISConfig", tool_ctx: "ToolContext") -> None:
        self._config = config
    ...
    def _resolve_openclaw_skills_dir(self) -> Path | None:
        ...
        configured = [
            str(os.getenv("JARVIS_OPENCLAW_SKILLS_DIR") or "").strip(),
    ...
    def _register_builtin_executors(self) -> None:
        ...
        self._skill_marketplace.register_executor(
            "prometheus.web-scraper", _CallableSkillExecutor(_exec_web_scraper)
        )
        self._skill_marketplace.register_executor(
            "prometheus.code-executor", _CallableSkillExecutor(_exec_code)
        )
        self._skill_marketplace.register_executor(
            "prometheus.file-manager", _CallableSkillExecutor(_exec_file_manager)
        )
        self._skill_marketplace.register_executor(
            "prometheus.email-sender", _CallableSkillExecutor(_exec_email_sender)
        )
        self._skill_marketplace.register_executor(
            "prometheus.daily-briefing", _CallableSkillExecutor(_exec_daily_briefing)
        )
        self._skill_marketplace.register_executor(
            "prometheus.data-analyzer", _CallableSkillExecutor(_exec_data_analyzer)
        )
