"""
JARVIS Subsystem Factory — Single source of truth for initializing all subsystems.
...
# CLI-based providers that can't be used recursively inside MCP
_CLI_PROVIDERS = {"claude-code", "gemini-cli"}
...
class SubsystemFactory:
    """Single place to initialize all JARVIS subsystems from config.
        ...
        *,
        ...
        Args:
            ...
            skip_cli_providers: If True, exclude CLI-based LLM providers
                (claude-code, gemini-cli) from the collaboration LLM chain.
                Set to True when running inside MCP to prevent recursive spawns.
            home_dir: Override home directory (defaults to config.home_dir or ~/.prometheus).
        """
        bundle = SubsystemBundle()
        _home = home_dir or getattr(config, "home_dir", os.path.expanduser("~/.prometheus"))
    ...
    def _init_db_adapter(bundle: SubsystemBundle, config: Any, home_dir: str) -> None:
        ...
        The MCP subprocess contention is solved by JARVIS_MCP_SUBPROCESS env var
        + 30s busy_timeout (commits 2c41b34, 5c17df7). The socket server
        infrastructure (db_server.py, db_client.py, db_adapter.py) is ready
        for future use when in-process SDK MCP has bugs and subprocess is needed.
        """
        return
    ...
    def _init_desktop(bundle: SubsystemBundle, config: Any) -> None:
        desktop_enabled = str(
            os.getenv("JARVIS_ENABLE_DESKTOP_CONTROL", "1")
        ...
        try:
            try:
                ...
                from src.jarvis.operators.desktop import MacDesktopOperator  # type: ignore
    ...
    def _init_unified_memory(bundle: SubsystemBundle, config: Any, home_dir: str) -> None:
        try:
            try:
                from jarvis.memory import UnifiedMemoryStore
            except ImportError:
                from src.jarvis.memory import UnifiedMemoryStore  # type: ignore

            try:
                from jarvis.services.embedding import get_default_embedding_client
            except ImportError:
                from src.jarvis.services.embedding import get_default_embedding_client  # type: ignore
    ...
    def _init_legacy_memory(bundle: SubsystemBundle, config: Any) -> None:
        try:
            try:
                ...
                from src.jarvis.services.embedding import get_default_embedding_client  # type: ignore
            try:
                from jarvis._vendor.common.memory_service import MemoryService
            except ImportError:
    ...
    def _init_mcts(bundle: SubsystemBundle) -> None:
        ...
        # JARVIS_ENABLE_MCTS_PLANNER=1 to bring it back.
        enabled = str(
            os.getenv("JARVIS_ENABLE_MCTS_PLANNER", "")
        ).strip().lower() in {"1", "true", "yes", "on"}
        if not enabled:
            logger.debug(
                "MCTS planner skipped (JARVIS_ENABLE_MCTS_PLANNER unset). "
        ...
        try:
            try:
                from jarvis._vendor.core_strategic_engine.planner_mcts import ABMCTSPlanner
            except ImportError:
                from jarvis._vendor.core_strategic_engine.planner_mcts import ABMCTSPlanner  # type: ignore
    ...
    def _init_voice(bundle: SubsystemBundle) -> None:
        try:
            try:
                from jarvis.services.voice import VoiceInterface
            except ImportError:
                from src.jarvis.services.voice import VoiceInterface  # type: ignore
    ...
    def _init_llm_collab(
        ...
        try:
            try:
                ...
                from src.jarvis.llm_providers.router import LLMInterface  # type: ignore
    ...
    def _init_openclaw(bundle: SubsystemBundle, config: Any) -> None:
        openclaw_enabled = os.environ.get("JARVIS_ENABLE_OPENCLAW_BRIDGE", "0") == "1" or bool(
        ...
        try:
            try:
                from jarvis.agents.autonomy import OpenClawBridgeRuntime
            except ImportError:
                from src.jarvis.agents.autonomy import OpenClawBridgeRuntime  # type: ignore
    ...
    def _init_sandbox(bundle: SubsystemBundle, config: Any) -> None:
        try:
            try:
                ...
                from src.jarvis.auth.sandbox import (  # type: ignore
                    DockerSandboxManager,
                    SubprocessSandboxManager,
                )
        ...
        # Sandbox pool
        try:
            from jarvis.auth.sandbox import SessionSandboxPool
