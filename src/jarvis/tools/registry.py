"""
JARVIS Tool Registry — Central registry for all tool definitions with metadata.
...
class ToolRegistry:
    """Central registry for all JARVIS tool definitions."""
    ...
    (
        {
            ...
            "description": (
                "Mark the first-run identity bootstrap as complete. "
                "Call this only after writing all 7 required identity files: "
                "IDENTITY, SOUL, USER, MEMORY, TOOLS, HEARTBEAT, REFLECTION. "
                "Will fail with a list of missing files if any are not yet written. "
                "Finalizes your birth and transitions to normal operation."
            ),
            "parameters": {
                "type": "object",
                "properties": {},
...
HIVEMIND_TOOLS_RAW = [
    ...
    (
        {
            "name": "skill_endorse",
            "description": "Endorse a pending crystallized skill for sharing with the Flame. Only endorsed skills can propagate to other JARVIS instances.",
            "parameters": {
                "type": "object",
                "properties": {
                    "genome_id": {
...
TRUST_POLICIES = {
    "yolo": {
        "description": "Maximum autonomy — JARVIS executes everything without asking",
...
def build_full_registry(
    *,
    ...
    if include_flame:
        _register_batch(reg, HIVEMIND_TOOLS_RAW)
    logger.info(
