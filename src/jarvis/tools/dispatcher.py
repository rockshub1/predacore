"""
JARVIS Tool Dispatcher — finds a handler and executes it with timeout/metrics.
...
from jarvis.services.rate_limiter import InMemoryBackend as _RateLimitBackend
from jarvis.auth.security import redact_secrets as _redact
from jarvis.auth.security import sanitize_tool_output as _sanitize_output
...
# ── Gemini CLI → JARVIS tool name mapping ─────────────────────────────
...
try:
    ...
    try:
        _TOOL_REQ = Counter(
            "jarvis_tool_requests_total", "Tool call count", ["tool_id", "status"]
        )
    except ValueError:
        _TOOL_REQ = None
    try:
        _TOOL_LAT = Histogram(
            "jarvis_tool_latency_seconds", "Tool call latency", ["tool_id"]
        )
    except ValueError:
        _TOOL_LAT = None
except ImportError:
    ...
    def __init__(
        ...
        *,
        ...
        self._rate_max = rate_max or int(
            os.getenv("JARVIS_TOOL_RATE_LIMIT", "120")
        )
        self._timeout = tool_timeout or int(
            os.getenv("JARVIS_TOOL_TIMEOUT_SECONDS", "600")
        )
    ...
    async def dispatch(
        ...
        *,
        ...
        _outer_timeout = float(
            os.getenv("JARVIS_TOOL_TIMEOUT_SECONDS", str(self._timeout))
        )
        t0 = time.time()
        try:
