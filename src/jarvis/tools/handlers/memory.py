...
def _normalize_scope(scope: str | None) -> str:
    try:
        from jarvis.memory.store import normalize_memory_scope
    except ImportError:
        from src.jarvis.memory.store import normalize_memory_scope  # type: ignore
...
async def handle_memory_store(args: dict[str, Any], ctx: ToolContext) -> str:
    ...
    try:
        mem = await ctx.memory_service.store(
            ...
            tags=tags, metadata={"key": key, "source": "jarvis.memory_store"},
            source="jarvis.memory_store",
        )
        return (
