"""Identity handlers: identity_read, identity_update, journal_append, bootstrap_complete."""
...
async def handle_identity_read(args: dict[str, Any], ctx: ToolContext) -> str:
    ...
    if not content:
        if engine.needs_bootstrap:
            raise ToolError(
                f"{filename} does not exist yet — complete the bootstrap first",
                kind=ToolErrorKind.NOT_FOUND,
                tool_name="identity_read",
                suggestion="Run bootstrap_complete first",
            )
...
async def handle_bootstrap_complete(args: dict[str, Any], ctx: ToolContext) -> str:
