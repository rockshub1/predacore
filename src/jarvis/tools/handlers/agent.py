...
def _team_memory_expiry(hours: Any) -> str | None:
    try:
        from jarvis.memory.store import future_iso_from_ttl
    except ImportError:
        from src.jarvis.memory.store import future_iso_from_ttl  # type: ignore
...
def _default_dynamic_agent_spec_inputs(
    ...
    *,
    ...
    try:
        from jarvis.agents.engine import AGENT_TYPES, CapabilityRouter
    except ImportError:
        from src.jarvis.agents.engine import AGENT_TYPES, CapabilityRouter  # type: ignore
...
async def _generate_dynamic_agent_specs(
    *,
    ...
    try:
        from jarvis.agents.engine import AGENT_TYPES, create_dynamic_agent_spec
    except ImportError:
        from src.jarvis.agents.engine import AGENT_TYPES, create_dynamic_agent_spec  # type: ignore
    ...
    meta_messages = [
        {
            "role": "system",
            "content": (
                "You are JARVIS's agent architect. Produce strict JSON only. "
...
async def _build_shared_multi_agent_prompt(
    ...
    *,
    ...
    if ctx.unified_memory is not None:
        try:
            try:
                from jarvis.memory import MemoryRetriever
            except ImportError:
                from src.jarvis.memory import MemoryRetriever  # type: ignore
...
async def handle_multi_agent(args: dict[str, Any], ctx: ToolContext) -> str:
    ...
    try:
        return await _multi_agent_inner(args, ctx)
    finally:
...
async def _multi_agent_inner(args: dict[str, Any], ctx: ToolContext) -> str:
    ...
    try:
        from jarvis.agents.collaboration import AgentSpec, AgentTeam, TaskPayload
        from jarvis.agents.engine import compile_dynamic_agent_prompt
    except ImportError:
        from src.jarvis.agents.collaboration import AgentSpec, AgentTeam, TaskPayload  # type: ignore
        from src.jarvis.agents.engine import compile_dynamic_agent_prompt  # type: ignore
...
async def _openclaw_delegate_inner(args: dict[str, Any], ctx: ToolContext) -> str:
    ...
    from jarvis.agents.autonomy import KillSwitchActivatedError
