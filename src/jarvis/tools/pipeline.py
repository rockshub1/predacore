"""
JARVIS Tool Pipeline — Chain multiple tools into composable workflows.
...
class _PipelineStateStore:
    ...
    def __init__(self, db_path: str | None = None, *, db_adapter=None):
        self._db_path = db_path or str(
            Path(os.getenv("JARVIS_HOME", "~/.prometheus")).expanduser() / "pipeline_states.db"
        )
    ...
    def save(self, state: dict[str, Any], expiry_sec: float = _TOKEN_EXPIRY_SEC) -> str:
        ...
        token = "jp_" + secrets.token_urlsafe(24)  # jp_ = jarvis pipeline
        now = time.time()
