"""
JARVIS Gateway — Central message router and session manager.

The Gateway is the "front door" of JARVIS. All messages from all
...
class Gateway:
    ...
    def __init__(
        ...
        Args:
            ...
            process_fn: Async function(user_id, message_text, session) -> response_text.
                        This is the JARVIS core brain function.
        """
        self.config = config
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            if engine.seed_exists and engine.is_bootstrapped:
    ...
    def _handle_model_command(
        ...
        known_providers = set(llm.available_providers) | set(PROVIDER_ENDPOINTS.keys()) | {
            "claude-code", "gemini-cli", "anthropic", "gemini",
        }
        ...
        try:
            ...
            response_text = (
                ...
                f"Switch back anytime with: /model {llm.available_providers[0] if llm.available_providers else 'claude-code'}"
            )
    ...
    async def handle_message(
        ...
        5. Get response from JARVIS core
