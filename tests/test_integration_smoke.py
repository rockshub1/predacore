...
def _mock_config():
    ...
    config.memory = types.SimpleNamespace(
        persistence_dir="/tmp/jarvis_test_memory",
    )
    config.home_dir = "/tmp/jarvis_test"
...
class TestDispatcherSmoke:
    ...
    def test_write_then_read(self, dispatcher, tmp_path):
        ...
        read_result = self._run(dispatcher.dispatch(
            "read_file", {"path": str(test_file)}, origin="test"
        ))
    ...
    def test_gemini_alias_run_in_terminal(self, dispatcher):
    ...
    def test_run_command_echo(self, dispatcher):
    ...
    def test_missing_param_error(self, dispatcher):
    ...
    def test_cache_invalidated_on_write(self, dispatcher, tmp_path):
    ...
    def test_execution_history_stats(self, dispatcher, tmp_path):
        """Execution history tracks stats correctly."""
        test_file = tmp_path / "stats.txt"
        test_file.write_text("stat me")

        # Run a few tool calls
        self._run(dispatcher.dispatch("read_file", {"path": str(test_file)}, origin="test"))
        self._run(dispatcher.dispatch("list_directory", {"path": str(tmp_path)}, origin="test"))
        self._run(dispatcher.dispatch("nonexistent_tool", {}, origin="test"))
