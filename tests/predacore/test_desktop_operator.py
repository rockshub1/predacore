...
from src.predacore.config import PredaCoreConfig, LLMConfig, MemoryConfig, SecurityConfig
from src.predacore.core import ToolExecutor
from src.predacore.operators.desktop import DesktopControlError, MacDesktopOperator
...
def test_open_app_respects_allowlist(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("PREDACORE_DESKTOP_ALLOWED_APPS", "Safari,Visual Studio Code")
    op = _mk_operator(monkeypatch)
...
@pytest.fixture
def config(tmp_path: Path) -> PredaCoreConfig:
...
@pytest.mark.asyncio
async def test_tool_executor_desktop_control_dispatch(config: PredaCoreConfig):
...
@pytest.mark.asyncio
async def test_tool_executor_desktop_control_disabled(config: PredaCoreConfig):
    executor = ToolExecutor(config)
