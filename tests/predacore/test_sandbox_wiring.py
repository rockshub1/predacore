"""
Tests for Docker sandbox wiring into PredaCore core ToolExecutor.
...
class PredaCoreConfig:
    ...
    mode: str = "cli"
    identity_dir: str = ""
    data_dir: str = "/tmp/predacore_test"
...
class TestFormatSandboxResult:
    ...
    def _import_tool_executor(self):
        ...
        with patch("src.predacore.core.PredaCoreConfig", PredaCoreConfig):
    ...
    def test_truncation(self):
        TE = self._import_tool_executor()
        # Truncation threshold was raised to 1 MB under the "remove all
        # limits" directive (executor.py::MAX_OUTPUT_CHARS). Generate
        # enough output to trigger it and verify the marker is inserted.
        from src.predacore.tools.executor import MAX_OUTPUT_CHARS
        huge_stdout = "x" * (MAX_OUTPUT_CHARS + 10_000)
        result = {"status": "SUCCESS", "stdout": huge_stdout, "stderr": "", "error_message": ""}
        out = TE._format_sandbox_result(result)
        assert "...[truncated]..." in out
        assert len(out) < len(huge_stdout)
...
class TestToolExecutorSandboxInit:
    ...
    def test_subprocess_sandbox_by_default(self):
        ...
        with patch("src.predacore.auth.sandbox.SubprocessSandboxManager") as MockSub:
            MockSub.return_value = MagicMock()
            with patch("src.predacore.auth.sandbox.DockerSandboxManager"):
                from src.predacore.core import ToolExecutor
    ...
    def test_docker_sandbox_when_configured(self):
        ...
        with patch("src.predacore.auth.sandbox.SubprocessSandboxManager") as MockSub:
            MockSub.return_value = MagicMock()
            with patch("src.predacore.auth.sandbox.DockerSandboxManager") as MockDocker:
...
class TestPythonExecTool:
    ...
    async def test_delegates_to_subprocess_sandbox(self):
        ...
        with patch("src.predacore.auth.sandbox.SubprocessSandboxManager", return_value=mock_sandbox):
            with patch("src.predacore.auth.sandbox.DockerSandboxManager"):
                from src.predacore.core import ToolExecutor
                te = ToolExecutor(config)
...
class TestExecuteCodeTool:
    ...
    async def test_requires_docker_for_non_python(self):
        ...
        config = PredaCoreConfig()
        with patch("src.predacore.auth.sandbox.SubprocessSandboxManager", return_value=MagicMock()):
            with patch("src.predacore.auth.sandbox.DockerSandboxManager"):
                from src.predacore.core import ToolExecutor
                te = ToolExecutor(config)
    ...
    async def test_delegates_to_docker_sandbox(self):
        ...
        with patch("src.predacore.auth.sandbox.SubprocessSandboxManager", return_value=MagicMock()):
            with patch("src.predacore.auth.sandbox.DockerSandboxManager", return_value=mock_docker):
                from src.predacore.core import ToolExecutor
                te = ToolExecutor(config)
...
class TestToolDefinitions:
    ...
    def test_execute_code_in_builtin_tools(self):
        from src.predacore.core import BUILTIN_TOOLS
