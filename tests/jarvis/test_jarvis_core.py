"""
🧪 JARVIS Test Suite — Identity System, Config, Sessions, Core

Tests the entire JARVIS stack from soul to tools:
...
from src.jarvis.identity.engine import get_identity_engine
from src.jarvis.prompts import _get_antigravity_prompt, _load_identity_file
from src.jarvis.identity.engine import reset_identity_engine
from src.jarvis.tools.handlers._context import ToolContext
from src.jarvis.tools.handlers.identity import handle_identity_read, handle_identity_update
from src.jarvis.tools.registry import TRUST_POLICIES, build_builtin_registry
from src.jarvis.services.lane_queue import LaneQueue
from src.jarvis.sessions import Session, SessionStore

# ── Paths ────────────────────────────────────────────────────────────

PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
IDENTITY_DIR = PROJECT_ROOT / "src" / "jarvis" / "identity"
...
@pytest.fixture
def config(tmp_home):
    """Create a JARVISConfig pointing to temp directories."""
...
class TestIdentityFileLoading:
    ...
    def test_loads_user_workspace_files(self, config, identity_dir):
        """USER.md and MEMORY.md load from user workspace (~/.prometheus/agents/{agent}/)."""
    ...
    def test_user_override_takes_priority(self, config, identity_dir):
        """User's ~/.prometheus/agents/{agent}/ files should override built-in."""
...
class TestSystemPromptAssembly:
    ...
    def test_prompt_without_antigravity(self, config):
        """Without Antigravity, no superpowers section."""
        config.antigravity.enabled = False
        prompt = _get_system_prompt(config)
        assert "Antigravity Superpowers" not in prompt

    def test_prompt_with_antigravity(self, config):
        """With Antigravity enabled, superpowers section appears."""
        config.antigravity.enabled = True
        prompt = _get_system_prompt(config)
        assert "Antigravity Superpowers" in prompt
        assert "ag_" in prompt
    ...
    def test_prompt_includes_self_meta_prompt_when_enabled(self, config):
        """Self-evolution mode should include self meta prompt text."""
        config.launch.enable_self_evolution = True
        prompt = _get_system_prompt(config)
        assert "JARVIS Self Meta Prompt" in prompt
...
class TestConfigSystem:
    """Tests for JARVISConfig — defaults, YAML, ENV overrides."""
    ...
    def test_sub_dirs_computed(self):
        ...
        cfg = JARVISConfig(home_dir="/tmp/test_prom")
    ...
    def test_antigravity_config_defaults(self):
        """Antigravity should be disabled by default."""
        ag = AntigravityConfig()
    ...
    def test_launch_profile_config_defaults(self):
        ...
        assert lp.max_tool_iterations == 10
    ...
    def test_load_config_public_profile_from_env(self, tmp_path, monkeypatch):
        ...
        cfg_file.write_text("name: JARVIS\n")

        monkeypatch.setenv("JARVIS_PROFILE", "public_beast")
    ...
    def test_load_config_persona_drift_env_overrides(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("JARVIS_ENABLE_PERSONA_DRIFT_GUARD", "1")
        monkeypatch.setenv("JARVIS_PERSONA_DRIFT_THRESHOLD", "0.35")
        monkeypatch.setenv("JARVIS_PERSONA_DRIFT_MAX_REGENS", "3")
...
class TestSessionPersistence:
    ...
    def test_session_add_message(self):
        ...
        s = Session()
        s.add_message("user", "Hello JARVIS")
...
class TestToolExecutor:
    """Tests for the ToolExecutor — JARVIS's hands."""
    ...
    async def test_memory_store_and_recall(self, config):
        ...
        await executor.execute(
            "memory_store",
            {"key": "test_fact", "content": "JARVIS is awesome"},
        )
...
class TestCoreLaunchBehavior:
    ...
    def test_core_exposes_openclaw_tool_when_enabled(self, tmp_home, monkeypatch):
        ...
        # Clean up env vars set by JARVISCore.__init__ to prevent test pollution
        monkeypatch.delenv("JARVIS_ENABLE_OPENCLAW_BRIDGE", raising=False)
        monkeypatch.delenv("JARVIS_ENABLE_PLUGIN_MARKETPLACE", raising=False)
    ...
    async def test_regenerates_when_drift_exceeds_threshold(self, config):
        ...
        core.llm.chat = AsyncMock(
            side_effect=[
                ...
                {
                    "content": "I am JARVIS from Project Prometheus. I can run tools.",
                    "tool_calls": [],
...
class TestSoulSeedContent:
    ...
    def test_has_security_invariants(self, seed_content):
        ...
        assert "Security Invariants" in seed_content
    ...
    def test_has_memory_rules(self, seed_content):
        """SOUL_SEED.md should have memory rules."""
        assert "Memory" in seed_content
...
class TestIntegration:
    ...
    def test_full_prompt_assembly(self, config):
        ...
        # Should contain content from identity seed
        assert "Cannot Change" in prompt or "Security Invariants" in prompt
...
# ══════════════════════════════════════════════════════════════════════
# TEST GROUP 13: Antigravity Prompt Section
# ══════════════════════════════════════════════════════════════════════


class TestAntigravityPrompt:
    """Tests for the Antigravity superpowers prompt section."""

    def test_disabled_returns_empty(self, config):
        """Antigravity disabled should return empty string."""
        config.antigravity.enabled = False
        result = _get_antigravity_prompt(config)
        assert result == ""

    def test_enabled_returns_content(self, config):
        """Antigravity enabled should return superpowers section."""
        config.antigravity.enabled = True
        result = _get_antigravity_prompt(config)
        assert "Superpowers" in result
        assert "ag_" in result

    def test_mentions_cross_session(self, config):
        """Antigravity prompt should mention cross-session search."""
        config.antigravity.enabled = True
        result = _get_antigravity_prompt(config)
        assert "Cross-Session" in result or "search" in result.lower()

    def test_mentions_agent_delegation(self, config):
        """Antigravity prompt should mention agent delegation."""
        config.antigravity.enabled = True
        result = _get_antigravity_prompt(config)
...
class TestEdgeCases:
    ...
    def test_identity_files_exist(self):
        ...
        assert (IDENTITY_DIR / "BOOTSTRAP.md").exists(), "BOOTSTRAP.md missing!"
    ...
    def test_identity_files_are_utf8(self):
        ...
        for name in ["SOUL_SEED.md", "BOOTSTRAP.md"]:
