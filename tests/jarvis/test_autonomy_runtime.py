...
from src.jarvis.agents.autonomy import (
...
@pytest.fixture
def runtime_config(tmp_path: Path) -> JARVISConfig:
...
@pytest.mark.asyncio
async def test_delegate_uses_idempotency_cache(runtime_config: JARVISConfig):
...
@pytest.mark.asyncio
async def test_delegate_openresponses_payload_shape(runtime_config: JARVISConfig):
    runtime = OpenClawBridgeRuntime(runtime_config)
...
@pytest.mark.asyncio
async def test_delegate_polls_until_terminal_state(runtime_config: JARVISConfig):
...
async def test_delegate_blocked_by_kill_switch(
    ...
    monkeypatch.setenv("JARVIS_KILL_SWITCH", "1")
