...
async def test_openclaw_skills_autodetects_installed_cli(_config, tmp_path, monkeypatch):
    from src.jarvis.tools.marketplace import MarketplaceManager

    monkeypatch.delenv("JARVIS_OPENCLAW_SKILLS_DIR", raising=False)
    ...
    monkeypatch.setattr("src.jarvis.tools.marketplace.shutil.which", lambda name: str(bin_dir / "openclaw"))
...
async def test_openclaw_skills_no_longer_falls_back_to_vendored_tree(_config, tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("OPENCLAW_SKILLS_DIR", raising=False)
    _config.openclaw.skills_dir = ""
    monkeypatch.setattr("src.jarvis.tools.marketplace.shutil.which", lambda name: None)
