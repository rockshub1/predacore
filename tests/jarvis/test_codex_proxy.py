...
from src.jarvis.evals import codex_proxy
...
def test_build_tool_prompted_uses_mcp_without_text_tool_instructions():
    ...
    assert "Jarvis tools are connected via MCP" in prompt
    assert "TOOL USE INSTRUCTIONS" not in prompt
    assert "<tool_call>" not in prompt


def test_jarvis_mcp_overrides_include_command_args_and_pythonpath(monkeypatch):
    monkeypatch.setenv("PYTHONPATH", "/tmp/existing")
    overrides = codex_proxy._jarvis_mcp_overrides()
...
def test_chat_completions_uses_mcp_mode_for_tool_requests(monkeypatch):
    ...
    monkeypatch.setattr(codex_proxy, "USE_JARVIS_MCP", True)
