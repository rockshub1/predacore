"""
End-to-end smoke tests for the full JARVIS pipeline.
...
from src.jarvis.gateway import (
    ChannelAdapter,
    Gateway,
    IncomingMessage,
    OutgoingMessage,
)
from src.jarvis.sessions import Message, Session, SessionStore
from src.jarvis.tools.dispatcher import ToolDispatcher
from src.jarvis.tools.handlers._context import ToolContext
from src.jarvis.tools.trust_policy import TrustPolicyEvaluator
...
def _make_config(tmp_home: Path) -> JARVISConfig:
    """Create a JARVISConfig rooted in a temp directory."""
...
def _make_dispatcher(config: JARVISConfig) -> ToolDispatcher:
...
@pytest.fixture
def config(tmp_home):
    """JARVISConfig pointing to temp directories."""
...
class TestDaemonStartup:
    ...
    def test_config_loads_with_defaults(self, config):
        """JARVISConfig loads with sane defaults from temp dir."""
        assert config.name == "JARVIS-Test"
...
class TestToolDispatch:
    ...
    def test_read_file_via_dispatcher(self, dispatcher, tmp_path):
        ...
        f.write_text("JARVIS smoke test content")

        result = _run(dispatcher.dispatch(
            "read_file", {"path": str(f)}, origin="test"
        ))
        assert "JARVIS smoke test content" in result
...
class TestMemoryStoreAndRecall:
    ...
    def memory_store(self, tmp_path):
        ...
        from src.jarvis.memory.store import UnifiedMemoryStore
    ...
    def test_store_and_recall_basic(self, memory_store):
        ...
        mem_id = _run(memory_store.store(
            content="JARVIS was created as part of Project Prometheus",
            memory_type="fact",
            importance=3,
            tags=["project", "origin"],
            user_id="test_user",
    ...
    def test_store_multiple_recall_ordered(self, memory_store):
        ...
        _run(memory_store.store(
            content="Python is the primary language for JARVIS",
        ...
        _run(memory_store.store(
            content="JARVIS uses SQLite for persistent memory storage",
            memory_type="fact", importance=3, user_id="test_user",
        ))

        results = _run(memory_store.recall(
            query="JARVIS", user_id="test_user", top_k=5
        ))
        # Should find at least the two JARVIS-related memories
        contents = [r[0]["content"] for r in results]
        jarvis_hits = [c for c in contents if "JARVIS" in c]
        assert len(jarvis_hits) >= 1
    ...
    def test_memory_handler_via_dispatcher(self, config, tmp_path):
        ...
        result = _run(disp.dispatch(
            "memory_store",
            {"key": "test_key", "content": "JARVIS is cool", "tags": ["test"]},
            origin="test",
        ))
        ...
        assert "cool" in result.lower() or "jarvis" in result.lower()
...
class TestSessionManagement:
    ...
    def test_add_messages_persist(self, tmp_path):
        ...
        assert first["content"] == "Hello JARVIS"
...
class TestErrorHandling:
    ...
    def test_tool_file_not_found(self, dispatcher):
        ...
        result = _run(dispatcher.dispatch(
            "read_file", {"path": "/tmp/definitely_does_not_exist_jarvis_test.txt"},
            origin="test",
        ))
    ...
    def test_tool_timeout_handling(self, config):
        ...
        from src.jarvis.tools.handlers import HANDLER_MAP
    ...
    def test_circuit_breaker_starts_closed(self, dispatcher):
        ...
        from src.jarvis.tools.resilience import CircuitState
...
class TestPersonaDriftGuard:
    ...
    def test_empty_response_scores_high_drift(self, core_config):
        ...
        with patch("src.jarvis.core.LLMInterface"), \
             patch("src.jarvis.core.ToolExecutor"), \
             patch("src.jarvis.core._get_system_prompt", return_value="test prompt"), \
             patch("src.jarvis.core.TranscriptWriter"), \
             patch("src.jarvis.core.OutcomeStore"):
            core = JARVISCore(core_config)
    ...
    def test_normal_response_passes_drift_check(self, core_config):
        """A normal JARVIS response should not trigger drift."""
        from src.jarvis.core import JARVISCore
...
class TestInProcessMCP:
    ...
    def test_mcp_tool_definitions_load(self):
        ...
        from src.jarvis.tools.mcp_server import _get_tool_definitions
    ...
    def test_mcp_tool_names_have_prefix(self):
        ...
        from src.jarvis.tools.mcp_server import get_mcp_tool_names

        names = get_mcp_tool_names()
        assert len(names) > 0
        for name in names:
            assert name.startswith("mcp__jarvis__")

    def test_mcp_config_dict_structure(self):
        """get_mcp_config_dict returns valid MCP config."""
        from src.jarvis.tools.mcp_server import get_mcp_config_dict
    ...
    def test_mcp_handler_factory(self, dispatcher):
        ...
        from src.jarvis.tools.mcp_server import _make_tool_handler
    ...
    def test_mcp_stdio_server_message_handling(self):
        ...
        from src.jarvis.tools.mcp_server import MCPStdioServer
    ...
    def test_build_sdk_mcp_server_structure(self, dispatcher):
        ...
        from src.jarvis.tools.mcp_server import build_sdk_mcp_server
